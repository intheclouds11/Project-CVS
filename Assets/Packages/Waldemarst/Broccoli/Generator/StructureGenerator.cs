...
namespace Broccoli.Generator
{
	...
	public class StructureGenerator {
		...
		public class StructureLevel {
			...
			/// </summary>
			[Range (0f,1f)]
			public float normalSproutRandomness = 0f;
			/// <summary>
			...
			/// <summary>
			/// Flag to override the frequency limit.
			/// </summary>
			public bool overrideFrequencyLimit = false;
			/// <summary>
			/// Overrided frequency limit.
			/// </summary>
			public int frequencyLimit = 30;
			/// <summary>
			/// Object to bind to this structure level.
			/// </summary>
			[System.NonSerialized]
			...
			#region Events
			/// <summary>
			/// Called before a StructureLevel will be processed for a Structure.
			/// </summary>
			/// <param name="structureLevel">StructureLevel instance to generate structures from.</param>
			/// <param name="parentStructure">Structure that will receive the structures.</param>
			public delegate void OnBeforeGenerateStructures (StructureLevel structureLevel, Structure parentStructure);
			/// <summary>
			/// Called after a StructureLevel has generated structures for a structure.
			/// </summary>
			/// <param name="structureLevel">StructureLevel instance to generate structures from.</param>
			/// <param name="parentStructure">Structure that will receive the structures.</param>
			/// <param name="structures"></param>
			public delegate void OnGenerateStructures (StructureLevel structureLeve, Structure parentStructure, List<Structure> structures);
			/// <summary>
			/// Called before generating structures.
			/// </summary>
			public OnBeforeGenerateStructures onBeforeGenerateStructures;
			/// <summary>
			/// Called after generating structures.
			/// </summary>
			public OnGenerateStructures onGenerateStructures;
			#endregion
			...
			/// <summary>
			/// Gets the first sprout structure level that has assigned the requested group id.
			/// </summary>
			/// <param name="groupId">Group id assigned to the sprout level.</param>
			/// <returns>Structure level instance of null if not found.</returns>
			public StructureLevel GetSproutStructureLevelByGroupId (int groupId) {
				for (int i = 0; i < structureLevels.Count; i++) {
					if (structureLevels[i].isSprout && structureLevels[i].sproutGroupId == groupId) {
						return structureLevels[i];
					}
				}
				return null;
			...
			public StructureLevel Clone() {
				...
				clone.sproutGroupColor = sproutGroupColor;
				...
				clone.overrideFrequencyLimit = overrideFrequencyLimit;
				clone.frequencyLimit = frequencyLimit;
				return clone;
			}
			#endregion
			...
			public string GetDebugInfo () {
				...
				info += string.Format ("minFreq: {0}, maxFreq: {1}, overrideFreqLimit: {2}, freqLimit: {3}\n", 
					minFrequency, maxFrequency, overrideFrequencyLimit, frequencyLimit);
		...
		private List<int> tmpReferencesIds = new List<int> ();
		/// <summary>
		/// The positions.
		/// </summary>
		...
		/// <param name="structureLevel">StructureLevel instance to generate structures from.</param>
		/// <param name="parentStructure">Structure that will receive the structures.</param>
		public delegate void OnBeforeGenerateStructures (List<Structure> structures, StructureLevel structureLevel, Structure parentStructure);
		/// <summary>
		/// Called after a StructureLevel has generated structures for a structure.
		/// </summary>
		/// <param name="structureLevel">StructureLevel instance to generate structures from.</param>
		/// <param name="parentStructure">Structure that will receive the structures.</param>
		/// <param name="structures"></param>
		public delegate void OnGenerateStructures (StructureLevel structureLevel, Structure parentStructure, List<Structure> structures);
		/// <summary>
		/// Called before generating structures.
		/// </summary>
		public OnBeforeGenerateStructures onBeforeGenerateStructures;
		...
		private void PrepareStructuresRecursive (List<Structure> structures) {
			...
			for (int i = 0; i < structures.Count; i++) {
				/*
				if (structures[i].id >= lastStructureId) {
					lastStructureId = structures[i].id;
				}
				*/
		...
		private int GetNextStructureId () {
			int candidateId = 0;
			while (existingStructureId.Contains (candidateId) || tmpReferencesIds.Contains (candidateId)) {
				candidateId++;
			}
			lastStructureId = candidateId;
			tmpReferencesIds.Add (candidateId);
			return candidateId;
		}
		/// <summary>
			...
			Structure parentStructure) 
		{
			...
			structureLevel.onBeforeGenerateStructures?.Invoke (structureLevel, parentStructure);
			...
			if (tunedStructures == 0) { // No tuned structures found, meaning we will create anew
				// If level is disabled, return.
				if (!structureLevel.enabled) return structures;
				...
				if (originStructureLevel.isSprout) {
					...
					sproutStructure.probability = structureLevel.probability;
					sproutStructure.enabled = structureLevel.enabled;
					sproutStructure.minFrequency = structureLevel.minFrequency;
					sproutStructure.maxFrequency = structureLevel.maxFrequency;
					...
					structureLevel.onGenerateStructures?.Invoke (structureLevel, parentStructure, structures);
					onGenerateStructures?.Invoke (structureLevel, parentStructure, structures);
					return structures;
				}
			...
			structureLevel.onGenerateStructures?.Invoke (structureLevel, parentStructure, mergedStructures);
			onGenerateStructures?.Invoke (structureLevel, parentStructure, structures);
		...
		public void BuildTree (BroccoTree tree, List<Structure> structures) {
			...
			tree.Clear ();
			for (int i = 0; i < structures.Count; i++) {
				tree.AddBranch (structures[i].branch, structures[i].branch.originOffset);
		...
		private void GenerateBranchesAndSproutsRecursive (Structure parentStructure) {
			...
			for (int i = 0; i < parentStructure.childrenTerminalStructures.Count; i++) {
				if (idToStructureLevel.ContainsKey (parentStructure.childrenTerminalStructures[i].generatorId)) {
					...
					levelSprouts = GenerateSproutLevel (parentStructure.branch, level, parentStructure.childrenTerminalStructures[i], out occurred);
				...
				if (structureLevel.enabled && structureLevel.probability >= probability) {
					...
					if (preferredFrequency > 0) {
						...
						spawnedSprouts = SproutGenerator.GetSprouts (preferredFrequency, childrenPerNode,
							...
							structureLevel.flipSproutAlign, structureLevel.flipSproutDirection, structureLevel.normalSproutRandomness,
		...
		public bool usePositionRoundRobin = true;
		...
		protected void SetRootPositionAndDirection (Structure structure, StructureLevel rootStructureLevel) {
			...
			if (positions.Count > 0) {
				...
				structure.branch.originOffset = structure.positionOffset;
				if (positions [index].overrideRootDirection) {
					structure.branch.ApplyDirectionalLength (positions [index].rootDirection, structure.branch.length);
				}
			}
			// Set trunk position from a random circle. 
			else {
			...
			out bool occurred) 
		{
			...
			float probability = Random.Range (0f, 1f);
			if (terminalStructure.enabled && terminalStructure.probability >= probability) {
				level.isVisitedAndActive = true;
				occurred = true;
				branch.Update ();
				branch.RecalculateNormals ();
				int frequency = Random.Range (terminalStructure.minFrequency, terminalStructure.maxFrequency + 1);
				if (frequency > 0) {
					...
					spawnedSprouts = SproutGenerator.GetSprouts (frequency, childrenPerNode,
						...
						level.flipSproutAlign, level.flipSproutDirection, level.normalSproutRandomness,
