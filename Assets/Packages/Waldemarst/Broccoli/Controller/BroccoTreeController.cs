...
namespace Broccoli.Controller {
	...
	/// Deprecated: legacy controller for trees created with Broccoli up to version 1.8.2.
	/// </summary>
	[ExecuteInEditMode]
	public class BroccoTreeController : MonoBehaviour {
		#region Vars
		/// <summary>
		...
		private float windMain = 0f;
		private float leafSwayFactor = 1f;
		private float leafTurbulenceFactor = 1f;
		public float sproutTurbulance = 1f;
		...
		void Update () {
			if (_renderer != null && _renderer.isVisible && hasSpeedTreeWind) {
				#if UNITY_EDITOR
				if (editorWindEnabled || EditorApplication.isPlaying) {
					UpdateSpeedTreeWind ();
				}
				#else
				UpdateSpeedTreeWind ();
		...
		public void SetupSpeedTreeWind (bool enable = true) {
			...
			_ST_WindBranch.y *= _BranchRipple;			// fDistance
			_ST_WindBranch.x *= _BranchRippleSpeed;		// fTime
			...
			vWindData.x(z)		// Branch fWeight
			vWindData.y(w)		// Branch fOffset
			*/
			if (_propBlock == null) return;
			_renderer.GetPropertyBlock (_propBlock);
			enable &= GetWindZoneValues ();
			if (!enable) {
			...
			valueSTWindGlobal = new Vector4 (Time.time * 0.36f, 
				baseWindAmplitude * localWindAmplitude * globalWindAmplitude * windMain, 
				trunkBending * 0.15f + 0.001f, windGlobalW);
			...
			valueSTWindLeaf1Tumble = new Vector4 (Time.time* 0.84f, 0.1298f, 0.11403f, 0.11f) * leafSwayFactor;
			_propBlock.SetVector (propSTWindLeaf1Tumble, valueSTWindLeaf1Tumble);
			// STWindLeaf2Tumble (time, 0.1, 0.1, 0.1)
			valueSTWindLeaf2Tumble = new Vector4 (Time.time, 0.035f, 0.035f, 0.5f) * leafSwayFactor;
			_propBlock.SetVector (propSTWindLeaf2Tumble, valueSTWindLeaf2Tumble);
			// STWindLeaf1Twitch (0.3, 0.3, time * 1.5, 0.0)
			valueSTWindLeaf1Twitch = new Vector4 (0.3315f, 0.3246f, Time.time * 1.56f, 0f) * leafTurbulenceFactor;
			_propBlock.SetVector (propSTWindLeaf1Twitch, valueSTWindLeaf1Twitch);
			// STWindLeaf2Twitch (0, 33.3, time / 1.5, 0.0)
			valueSTWindLeaf2Twitch = new Vector4 (0.01745f, 33.3333f, Time.time * 0.31f, 12.896f) * leafTurbulenceFactor;
		...
		public void UpdateSpeedTreeWind () {
			...
			valueSTWindGlobal.z = trunkBending * 0.15f + 0.001f;
			valueSTWindGlobal.w = windGlobalW;
			_propBlock.SetVector (propSTWindGlobal, valueSTWindGlobal);
			// STWindBranch
			valueSTWindBranch = new Vector4 (valueTime * 0.65f, 0.4102f, valueTime * 1.5f, 0f) * localWindAmplitude; // Branch swaying
			_propBlock.SetVector (propSTWindBranch, valueSTWindBranch);
			// STWindLeaf1Ripple (time * 3.2, 0, 0.5, 0)
			valueSTWindLeaf1Ripple = new Vector4 (valueTime * 3.18f, 0.044f, 0.5f, 0f) * windScale * sproutTurbulance; // Leaf Ripple
			_propBlock.SetVector (propSTWindLeaf1Ripple, valueSTWindLeaf1Ripple);
			// STWindLeaf2Ripple (time * 4.7, 0, 0.5, 0)
			valueSTWindLeaf2Ripple = new Vector4 (valueTime * 4.7f, 0f, 0.5f, 0f) * windScale * sproutTurbulance; // Leaf Ripple
			_propBlock.SetVector (propSTWindLeaf2Ripple, valueSTWindLeaf2Ripple);
			// STWindLeaf1Tumble (time, 0.1, 0.1, 0.1)
			valueSTWindLeaf1Tumble = new Vector4 (valueTime * 0.84f, 0.1298f, 0.11403f, 0.11f) * leafSwayFactor * sproutSway; // EFFECT SPROUT SWAY
			_propBlock.SetVector (propSTWindLeaf1Tumble, valueSTWindLeaf1Tumble);
			// STWindLeaf2Tumble (time, 0.1, 0.1, 0.1)
			valueSTWindLeaf2Tumble = new Vector4 (valueTime, 0.035f, 0.035f, 0.5f) * leafSwayFactor * sproutSway; // EFFECT SPROUT SWAY
			_propBlock.SetVector (propSTWindLeaf2Tumble, valueSTWindLeaf2Tumble);
			// STWindLeaf1Twitch (0.3, 0.3, time * 1.5, 0.0)
			valueSTWindLeaf1Twitch = new Vector4 (0.3315f, 0.3246f, valueTime * 1.56f, 0f) * leafTurbulenceFactor * sproutSway; // EFFECT SPROUT TURBULENCE
			_propBlock.SetVector (propSTWindLeaf1Twitch, valueSTWindLeaf1Twitch);
			// STWindLeaf2Twitch (0, 33.3, time / 1.5, 0.0)
			valueSTWindLeaf2Twitch = new Vector4 (0.01745f, 33.3333f, valueTime * 0.31f, 12.896f) * leafTurbulenceFactor * sproutSway; // EFFECT SPROUT TURBULENCE
			_propBlock.SetVector (propSTWindLeaf2Twitch, valueSTWindLeaf2Twitch);
			// STWindFrondRipple (time * -40, 1.2, 10.3, 0)
		...
		/// <summary>
		/// Get the wind parameters from the first WindZone object in the scene.
		/// </summary>
		/// <returns><c>True</c> if the WindZone object was found.</returns>
		public bool GetWindZoneValues () {
			if (shaderType == ShaderType.SpeedTree8OrCompatible) {
				leafSwayFactor = 0.4f;
				leafTurbulenceFactor = 0.4f;
			}
			...
			for (int i = 0; i < windZones.Length; i++) {
				if (windZones [i].gameObject.activeSelf && windZones[i].mode == WindZoneMode.Directional) {
					...
					leafSwayFactor *= windZones [i].windMain;
					leafTurbulenceFactor *= windZones [i].windTurbulence;
					return true;
				}
			}
			return false;
		...
		public void EnableEditorWind (bool enabled) {
			#if UNITY_EDITOR
