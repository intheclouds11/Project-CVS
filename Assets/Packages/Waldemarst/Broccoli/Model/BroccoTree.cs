...
namespace Broccoli.Model {
	...
	public class BroccoTree {
		...
		public class Branch {
			...
			void InternalOnLengthChanged (BezierCurve curve, float oldLength, float newLength) {
				for (int i = 0; i < _branches.Count; i++) {
					_branches [i].UpdateAccumLength ();
			...
			public void UpdatePosition () {
				if (_parent != null) {
					...
					if (isFollowUp) {
						...
						originOffset = _parent.originOffset + _parent.curve.GetPointAt(1f).position - curve.GetPointAt(0f).position;
				...
				// If trunk.
					...
					parentOffset = Vector3.zero;
					if (GlobalSettings.trunkToOriginOffsetEnabled) {
						CurvePoint originPoint = curve.GetPointAt (0f);
						originOffset = -originPoint.position;
					}
				}
				for (int i = 0; i < _branches.Count; i++) {
					_branches[i].UpdatePosition ();
			...
			public List<Branch> GetDescendantBranches (List<BroccoTree.Branch> childrenBranches = null) {
				if (childrenBranches == null) {
					childrenBranches = new List<Branch> ();
				}
				childrenBranches.Add (this);
				for (int i = 0; i < _branches.Count; i++) {
					_branches[i].GetDescendantBranches (childrenBranches);
				}
			...
			public void UpdateResolution (int stepsPerUnit, bool recursive = false) {
				curve.resolution = stepsPerUnit;
				curve.ComputeSamples ();
				if (recursive) {
					for (int i = 0; i < branches.Count; i++) {
						branches [i].UpdateResolution (stepsPerUnit, true);
					}
				}
			}
			/// <summary>
			...
			/// <summary>
			/// Gets the surface branch point normal given a relative position and roll angle in radians.
			/// </summary>
			/// <param name="position">Relative position (0-1) at a branch length.</param>
			/// <param name="rollAngle">Roll angle in radians.</param>
			/// <returns>Surface point normal on this branch.</returns>
			public Vector3 GetSurfaceNormalAt (float position, float rollAngle) {
				if (_shaper != null) {
					return _shaper.GetSurfaceNormalAt (position, rollAngle, this);
				}
				return GetNormalAtPosition (position, rollAngle);
			}
			/// <summary>
		...
		/// <summary>
		/// Updates the curve resolution on every branch of the hierarchy according to the steps per length unit value.
		/// </summary>
		/// <param name="stepsPerUnit">How many resolution steps to get per length unit.</param>
		public void UpdateResolution (int stepsPerUnit) {
			for (int i = 0; i < _branches.Count; i++) {
				_branches[i].UpdateResolution (stepsPerUnit, true);
			}
		}
		/// <summary>
		/// Recalculates the normals.
		...
		public List<Branch> GetDescendantBranches () {
			List<Branch> childrenBranches = new List<Branch> ();
			foreach (Branch child in _branches) {
				child.GetDescendantBranches (childrenBranches);
			}
			return childrenBranches;
		}
		/// <summary>
