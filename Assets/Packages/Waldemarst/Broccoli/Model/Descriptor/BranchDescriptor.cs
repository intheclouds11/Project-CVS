...
namespace Broccoli.Pipe {
    ...
    public class BranchDescriptor
    {
        ...
        #region Sprout Group Descriptor
        [System.Serializable]
        public class SproutStructure: ISerializationCallbackReceiver
        {
            #region Vars
            ...
            public SproutMesh.ScaleMode scaleMode = SproutMesh.ScaleMode.Hierarchy;
            public float flipAlign = 0.8f;
            public float normalRandomness = 0.5f;
            public float bendingAtTop = 0f;
            public float bendingAtBase = 0f;
            public float sideBendingAtTop = 0f;
            public float sideBendingAtBase = 0f;
            /// <summary>
            /// The Sprout noise pattern.
            /// </summary>
            public SproutMesh.NoisePattern noisePattern = SproutMesh.NoisePattern.None;
            /// <summary>
            /// Noise mode to calculate the sprout mesh size.
            /// </summary>
            public SproutMesh.NoiseDistribution noiseDistribution = SproutMesh.NoiseDistribution.Hierarchy;
            /// <summary>
            ...
            public List<SproutLevelDescriptor> levelDescriptors = new List<SproutLevelDescriptor> ();
            public Guid styleId = Guid.Empty;
            [SerializeField]
            private string _serializedStyleId = string.Empty;
            /*
            ...
            */
            ...
            /// Constructor.
            /// </summary>
            public SproutStructure (bool createLevelDescriptors)
            {
                if (createLevelDescriptors) {
                    // Adding 4 level descriptors.
                    for (int i = 0; i < 4; i++) {
                        levelDescriptors.Add (new SproutLevelDescriptor ());
                    }
                }
            }
            #endregion
			...
			{
				_serializedStyleId = styleId.ToString();
			}

			public void OnAfterDeserialize()
			{
				if (string.IsNullOrEmpty (_serializedStyleId)) {
					styleId = Guid.Empty;
                } else {
					styleId = new Guid(_serializedStyleId);
                }
			}
			#endregion
            ...
            public SproutStructure Clone ()
            {
                SproutStructure clone = new SproutStructure (false);
                ...
                clone.bendingAtTop = bendingAtTop;
                clone.bendingAtBase = bendingAtBase;
                clone.sideBendingAtTop = sideBendingAtTop;
                clone.sideBendingAtBase = sideBendingAtBase;
                ...
                clone.noiseResolutionCurve = new AnimationCurve (noiseResolutionCurve.keys);
                ...
                clone.noiseStrengthCurve = new AnimationCurve (noiseStrengthCurve.keys);
                for (int i = 0; i < levelDescriptors.Count; i++) {
                    clone.levelDescriptors.Add (levelDescriptors [i].Clone ());
                }
        ...
        public List<SproutStructure> sproutStructures = new List<SproutStructure> ();
        ...
        public BranchDescriptor Clone () {
            ...
            for (int i = 0; i < sproutStructures.Count; i++) {
                clone.sproutStructures.Add (sproutStructures[i].Clone ());
            }
        ...
        public string GetDebugInfo () {
            ...
            info += string.Format ("Noise Type: {0}\n", noiseType);
