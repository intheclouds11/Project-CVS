Shader "Hidden/Broccoli/BillboardNormal"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (float4 vertex : POSITION, float3 normal : NORMAL, float4 tangent : TANGENT, float2 uv : TEXCOORD0, float4 uv5 : TEXCOORD4, float4 color : COLOR)
            {
                v2f o;
                o.uv5 = uv5;
            ...
            float4 frag (v2f i, fixed facing : VFACE) : COLOR
            {
                ...
                color = color * 0.5 + 0.5;
                /*
                if (abs(i.uv5.w - 1.0) > 0.01) { // is branch 
                    color.a = 1;
