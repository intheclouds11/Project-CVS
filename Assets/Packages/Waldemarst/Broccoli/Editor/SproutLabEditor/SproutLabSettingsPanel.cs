...
namespace Broccoli.BroccoEditor
{
    public class SproutLabSettingsPanel {
		...
		private static string lightIntensityName = "var-light-intensity";
		private static string lightColorName = "var-light-color";
		...
		private static string gizmosOutlineColorName = "var-gizmos-outline-color";
		...
		private Slider lightIntensityElem;
		private ColorField lightColorElem;
		private Toggle gizmos3dElem;
		private Slider gizmos3dSizeElem;
		private Slider gizmosOutlineWidthElem;
		private ColorField gizmosOutlineColorElem;
        ...
        public void Initialize (SproutLabEditor sproutLabEditor) {
            ...
            if (!isInit) {
				...
				// Init light intensity field.
				lightIntensityElem = container.Q<Slider> (lightIntensityName);
				lightIntensityElem?.RegisterValueChangedCallback(evt => {
					float newVal = evt.newValue;
					if (listenGUIEvents && newVal != sproutLabEditor.branchDescriptorCollection.lightIntensity) {
						sproutLabEditor.branchDescriptorCollection.lightIntensity = evt.newValue;
						SproutLabEditor.RefreshSlider (lightIntensityElem, sproutLabEditor.branchDescriptorCollection.lightIntensity);
						sproutLabEditor.meshPreview.SetLightA (sproutLabEditor.branchDescriptorCollection.lightIntensity,
							sproutLabEditor.branchDescriptorCollection.lightColor);
					}
				});
				SproutLabEditor.SetupSlider (lightIntensityElem);

				// Init light color field.
				lightColorElem = container.Q<ColorField> (lightColorName);
				lightColorElem?.RegisterValueChangedCallback(evt => {
					Color newVal = evt.newValue;
					if (listenGUIEvents && newVal != sproutLabEditor.branchDescriptorCollection.lightColor) {
						sproutLabEditor.branchDescriptorCollection.lightColor = evt.newValue;
						sproutLabEditor.meshPreview.SetLightA (sproutLabEditor.branchDescriptorCollection.lightIntensity,
							sproutLabEditor.branchDescriptorCollection.lightColor);
					}
				});
				...
				// Gizmo outline color field.
				gizmosOutlineColorElem = container.Q<ColorField> (gizmosOutlineColorName);
				gizmosOutlineColorElem?.RegisterValueChangedCallback(evt => {
					Color newVal = evt.newValue;
					if (listenGUIEvents && newVal != sproutLabEditor.branchDescriptorCollection.gizmosOutlineColor) {
						sproutLabEditor.branchDescriptorCollection.gizmosOutlineColor = evt.newValue;
					}
				});
		...
		public void RefreshValues () {
			if (sproutLabEditor.branchDescriptorCollection != null) {
				...
				SproutLabEditor.RefreshSlider (lightIntensityElem, sproutLabEditor.branchDescriptorCollection.lightIntensity);
				lightColorElem.value = sproutLabEditor.branchDescriptorCollection.lightColor;
				...
				SproutLabEditor.RefreshSlider (gizmosOutlineWidthElem, sproutLabEditor.branchDescriptorCollection.gizmosOutlineWidth);
				gizmosOutlineColorElem.value = sproutLabEditor.branchDescriptorCollection.gizmosOutlineColor;
