...
namespace Broccoli.BroccoEditor
{
	...
	public class MeshPreview
	{
		...
		/// </summary>
		protected enum RenderPipelineType {
			Regular,
			HDRP,
			URP,
            LWRP
		...
		/// The SRP detected.
		/// </summary>
        protected RenderPipelineType detectedPipelineType = RenderPipelineType.Regular;
		#endregion
		...
		protected MeshPreview () {
			...
			_previewRenderUtility.ambientColor = new Color (1f, 1f, 1f, 1f);
		...
		/// <summary>
		/// Sets Light A intensity and color.
		/// </summary>
		/// <param name="intensity">Light intensity.</param>
		/// <param name="color">Light color.</param>
		public void SetLightA (float intensity, Color color) {
			SetLight (_lightA, intensity, color);
		}
		/// <summary>
		...
		/// <summary>
		/// Sets Light B intensity and color.
		/// </summary>
		/// <param name="intensity">Light intensity.</param>
		/// <param name="color">Light color.</param>
		public void SetLightB (float intensity, Color color) {
			SetLight (_lightB, intensity, color);
		}
		/// <summary>
		...
		/// <summary>
		/// Sets a ligth intensity and color.
		/// </summary>
		/// <param name="light">Light to set values.</param>
		/// <param name="intensity">Light intensity.</param>
		/// <param name="color">Light color.</param>
		void SetLight (Light light, float intensity, Color color) {
			light.intensity = intensity;
			light.color = color;
		}
		/// <summary>
		...
		private void PreRenderViewport (Rect r, GUIStyle background) {
			...
			#if BROCCOLI_DEVEL
			_previewRenderUtility.camera.gameObject.hideFlags = HideFlags.None;
			#endif
			...
			Unsupported.SetOverrideLightingSettings (currentScene);
		}
		/// <summary>
				...
				if (hasSecondPass) {
					if (pass == 1) {
						...
						if (firstPassTex == null) {
							...
							firstPassTex = RenderTexture.GetTemporary (resultRender.width, resultRender.height, 0, format);
		...
		/// Detects the SRP.
		/// </summary>
		protected void DetectRenderPipeline () {
		...
		void OnPostRender (Camera cam)
		{
			if (cam == _previewRenderUtility.camera) {
				if (hasSecondPass && currentPass == 2) {
					...
					if (blendTex == null) {
						GraphicsFormat format = camera.allowHDR ? GraphicsFormat.R16G16B16A16_SFloat : GraphicsFormat.R8G8B8A8_UNorm;
						blendTex = RenderTexture.GetTemporary (firstPassTex.width, firstPassTex.height, 0, format);
		...
		private void BuildPlaneMesh (float size, Vector3 position) {
			...
			_planeMaterial.SetFloat ("_Glossiness", 0.2f);
			_planeMaterial.SetFloat ("_Metallic", 0f);
		}
		/// <summary>
		...
		/// <param name="glossiness">Glossiness for the plane material.</param>
		/// <param name="metallic">Metallic for the plane material.</param>
		public void SetPlaneMesh (Vector3 scale, Color tint, float glossiness = 0.2f, float metallic = 0f) {
			_planeScale = scale;
			_planeTint = tint;
			if (_planeMaterial != null) {
				_planeMaterial.SetColor ("_Color", _planeTint);
				_planeMaterial.SetFloat ("_Glossiness", glossiness);
				_planeMaterial.SetFloat ("_Metallic", metallic);
			}
		}
		/// <summary>
