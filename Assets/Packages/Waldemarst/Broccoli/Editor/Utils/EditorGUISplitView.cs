...
namespace Broccoli.Utils
{
	public class EditorGUISplitView
	{
		...
		/// <summary>
		/// Calculated offset for every split.
		/// </summary>
		/// <typeparam name="int">Offset in pixels.</typeparam>
		...
		/// <summary>
		/// Size used when sizes where calculated.
		/// </summary>
		float calculatedAvailableSize = 0f;
		#endregion
		...
		/// <summary>
		/// Set a fixed size on a fixed split.
		/// </summary>
		/// <param name="index">Index for the fixed split.</param>
		/// <param name="size">Size in pixels for the split.</param>
		/// <returns>True if a fixed split is found at the given index.</returns>
		public bool SetFixedSplitSize (int index, float size) {
			if (index >= 0 && index < splits.Count && splits [index].mode == SplitDef.Mode.Fixed) {
				splits [index].sizePx = (int)size;
				splits [index].minSizePx = (int)size;
				limits [index] = 0;
				sizes [index] = 0;
				isCalculated = false;
				UpdateOffsets ();
		...
		/// <summary>
		/// Gets the split offset
		/// </summary>
		/// <returns></returns>
		public int GetCurrentSplitOffset () {
			if (currentSplitIndex >= 0) {
				return offset [currentSplitIndex];
			}
			return 0;
		}
		/// <summary>
		...
		public void BeginSplitView () {
			...
			if (availablePxSize > 0 && calculatedAvailableSize != availablePxSize) {
				isCalculated = false;
			}
			// Calculate widths.
		...
		void CalculateSizesAndLimits (float availablePxSize) {
			if (!isCalculated && availablePxSize > 0 && Event.current.type == EventType.Repaint) {
				isCalculated = true;
				calculatedAvailableSize = availablePxSize;
				float limit = 0;
				float calcPxSize = 0;
				float accumPxSize = 0f;
				SplitDef splitDef;
				for (int i = 0; i < splits.Count; i++) {
					splitDef = splits [i];
					if (splitDef.mode == SplitDef.Mode.Fixed) {
						calcPxSize = splitDef.sizePx;
						accumPxSize += calcPxSize;
					} else {
						calcPxSize = (availablePxSize - accumPxSize) * splitDef.size;
		...
		/// <summary>
		/// Calculates offsets per each split.
		/// </summary>
		void UpdateOffsets () {
			offset.Clear ();
			int accumOffset = 0;
			for (int i = 0; i < sizes.Count; i++) {
				offset.Add (accumOffset);
				accumOffset += sizes [i];
			}
		}
		/// <summary>
