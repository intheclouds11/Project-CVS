...
namespace Broccoli.TreeNodeEditor
{
    public class PipelineGraphView : GraphView {
		...
		private Color candidateEdgeColor = new Color (1f, 1f, 0.4f, 0.75f);
		private Color tmpCandidateEdgeColor = new Color (0.65f, 0.65f, 0.65f, 1f);
		#endregion
		...
		public Label headerElement;
		public Label footerElement;
        ...
        public void Init (Vector2 offset, float zoom) {
			...
			// Add header element.
			headerElement = new Label ();
			headerElement.AddToClassList (headerElementClassName);
			this.Add (headerElement);
			headerElement.style.display = DisplayStyle.None;

			// Add footer element.
			footerElement = new Label ();
			footerElement.AddToClassList (footerElementClassName);
			this.Add (footerElement);
			footerElement.style.display = DisplayStyle.None;
        }
		...
		public void SetHeader (string graphHeader) {
			if (headerElement != null) {
				if (string.IsNullOrEmpty (graphHeader)) {
					headerElement.style.display = DisplayStyle.None;
				} else {
					headerElement.style.display = DisplayStyle.Flex;
					headerElement.text = graphHeader;
				}
			}
		}
		public void SetFooter (string graphFooter) {
			if (footerElement != null) {
				if (string.IsNullOrEmpty (graphFooter)) {
					footerElement.style.display = DisplayStyle.None;
				} else {
					footerElement.style.display = DisplayStyle.Flex;
					footerElement.text = graphFooter;
				}
			}
		}
		...
		public override List<Port> GetCompatiblePorts (Port startPort, NodeAdapter nodeAdapter) {
			...
			_ports = ports.ToList().Where(endPort => {
					...
					// If cardinality does not match, return
					if (!res) return res;
					// If cardinality matches, check node weight.
					PipelineNode startNode = (PipelineNode)startPort.node;
					PipelineNode endNode = (PipelineNode)endPort.node;
					if (startPort.direction == Direction.Input) {
						if (startNode.pipelineElement.uniqueOnPipeline)
							return startNode.pipelineElement.positionWeight > endNode.pipelineElement.positionWeight;
						else
							return startNode.pipelineElement.positionWeight >= endNode.pipelineElement.positionWeight;
					} 
					else {
		...
		private PipelineElement GetPipelineElement (PipelineElement.ClassType classType) {
			...
			switch (classType) {
				case PipelineElement.ClassType.StructureGenerator: return ScriptableObject.CreateInstance<StructureGeneratorElement> ();
				case PipelineElement.ClassType.SproutGenerator: return ScriptableObject.CreateInstance<SproutGeneratorElement> ();
				case PipelineElement.ClassType.LengthTransform: return ScriptableObject.CreateInstance<LengthTransformElement> ();
				case PipelineElement.ClassType.GirthTransform: return ScriptableObject.CreateInstance<GirthTransformElement> ();
				case PipelineElement.ClassType.SparsingTransform: return ScriptableObject.CreateInstance<SparsingTransformElement> ();
				case PipelineElement.ClassType.BranchBender: return ScriptableObject.CreateInstance<BranchBenderElement> ();
				...
				case PipelineElement.ClassType.Positioner: return ScriptableObject.CreateInstance<PositionerElement> ();
				case PipelineElement.ClassType.Baker: return ScriptableObject.CreateInstance<BakerElement> ();
			}
		...
		private void ShowEdgeCandidates (PipelineNode pipelineNode) {
			bool isDisconnected = true;
			if ((pipelineNode.pipelineElement.connectionType == PipelineElement.ConnectionType.Source ||
				pipelineNode.pipelineElement.connectionType == PipelineElement.ConnectionType.Transform) &&
				pipelineNode.pipelineElement.sinkElement != null) {
					isDisconnected = false;	
			}
			else if ((pipelineNode.pipelineElement.connectionType == PipelineElement.ConnectionType.Sink ||
				pipelineNode.pipelineElement.connectionType == PipelineElement.ConnectionType.Transform) &&
				pipelineNode.pipelineElement.srcElement != null) {
					isDisconnected = false;
			}
			if (isDisconnected) {
				List<Edge> _edges = edges.ToList().Where(edge => {
					PipelineNode sinkNode = edge.input.node as PipelineNode;
					PipelineNode srcNode = edge.output.node as PipelineNode;
					bool canConnectUpstream;
					bool canConnectDownstream;
					if (srcNode.pipelineElement.uniqueOnPipeline) {
						canConnectUpstream = srcNode.pipelineElement.positionWeight < pipelineNode.pipelineElement.positionWeight;
					} else {
						canConnectUpstream = srcNode.pipelineElement.positionWeight <= pipelineNode.pipelineElement.positionWeight;
					}
					if (sinkNode.pipelineElement.uniqueOnPipeline) {
						canConnectDownstream = sinkNode.pipelineElement.positionWeight > pipelineNode.pipelineElement.positionWeight;
					} else {
						canConnectDownstream = sinkNode.pipelineElement.positionWeight >= pipelineNode.pipelineElement.positionWeight;
					}
					if (canConnectDownstream && canConnectUpstream) {
						//Debug.Log ($"src: {srcNode.pipelineElement.classType}, sink: {sinkNode.pipelineElement.classType}");
						return true;
					}
					return false;
				}).ToList();
				for (int i = 0; i < _edges.Count; i++) {
					Edge edge = _edges[i];
					tmpCandidateEdgeColor = edge.output.portColor;
					edge.output.portColor = candidateEdgeColor;
					edge.input.portColor = candidateEdgeColor;
					edge.MarkDirtyRepaint();
				}
			}
		}
		private void HideEdgeCandidates () {
			List<Edge> _edges = edges.ToList();
			for (int i = 0; i < _edges.Count; i++) {
					Edge edge = _edges[i];
					edge.output.portColor = tmpCandidateEdgeColor;
					edge.input.portColor = tmpCandidateEdgeColor;
					edge.MarkDirtyRepaint();
				}
		}
		...
		private void OnSelectNodeInternal (PipelineNode pipelineNode) {
			ShowEdgeCandidates (pipelineNode);
