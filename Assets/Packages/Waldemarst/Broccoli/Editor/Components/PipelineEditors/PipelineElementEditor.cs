...
namespace Broccoli.TreeNodeEditor
{
	...
	public abstract class PipelineElementEditor : Editor {
		...
		/// </summary>
		protected bool showDescriptionEnabled = false;
		/// <summary>
		/// Flag to let editors switch some slides to open float fields.
		/// </summary>
		protected bool offersOpenFields = false;
		/// <summary>
		/// Flag to display the button to turn off and on gizmos.
		/// </summary>
		protected bool showGizmosBtn = false;
		/// <summary>
		...
		protected static GUIContent showOpenFieldsLabel = new GUIContent ("Show Open Fields", 
			"Some properties are set with open float/int fields, instead of being restricted by sliders.");
		...
		public override void OnInspectorGUI () {
			BroccoEditorGUI.DrawHeader (
                pipelineElement.elementName, 
                pipelineElement.elementDescription, 
                pipelineElement.elementHelpURL, 
                headerColor, 
				showDescriptionEnabled,
				pipelineElement.showGizmosEnabled,
				out showDescriptionEnabled,
				out pipelineElement.showGizmosEnabled,
				true,
				showGizmosBtn,
				true);
		...
		#if BROCCOLI_DEVEL
		/// <summary>
		/// Displays general debug information for the selected element on the pipeline.
		/// </summary>
		protected void DrawDebugInfo () {
			string debugInfo = string.Format ("PipelineElement [{0}][{1}]\n", pipelineElement.elementName, pipelineElement.id);
			if (pipelineElement.pipeline == null) {
		...
		/// </summary>
		virtual public void DrawDebugInfoSpecific () {}
		#endif
		#endregion
	}
}