...
namespace Broccoli.TreeNodeEditor
{
	...
	public class StructureGeneratorElementEditor : PipelineElementEditor {
		...
		/// <summary>
		/// Keeps the editor width as reference to detect resizing.
		/// </summary>
		...
		private static GUIContent overrideNoiseLabel = new GUIContent ("Override Noise", "Set a factor to apply to noise base values.");
		private static GUIContent rootNoiseLabel = new GUIContent ("Noise Strength Factor", "Noise weight factor to apply to steam branches (trunks) noise base value.");
		private static GUIContent rootNoiseScaleLabel = new GUIContent ("Noise Scale Factor", "Noise scale factor to apply to steam branches (trunks) noise base value.");
		private static GUIContent noiseLabel = new GUIContent ("Noise Strength Factor", "Noise weight factor to apply to this branch or root noise base value.");
		private static GUIContent noiseScaleLabel = new GUIContent ("Noise Scale Factor", "Noise scale factor to apply to this branch or root noise base value.");
		...
		protected override void OnEnableSpecific () {
			...
			curveEditor.enableOverlayToolbar = false;
			curveEditor.rotateEnabled = false;
			curveEditor.scaleEnabled = false;
			curveEditor.OnEnable ();
			curveEditor.ShowToolbar ();
			/*
			...
			*/
			...
			curveEditor.onCheckDragNodes += OnCheckMoveNodes;

			// Move nodes.
			curveEditor.onBeginDragNodes += OnBeginMoveNodes;
			curveEditor.onDragNodes += OnMoveNodes;
			curveEditor.onEndDragNodes += OnEndMoveNodes;
			...
			// Move handles.
			curveEditor.onBeginDragHandle += OnBeginMoveHandle;
			curveEditor.onDragHandle += OnMoveHandle;
			curveEditor.onEndDragHandle += OnEndMoveHandle;
		...
		private void SetStructureInspectorEnabled (bool enabled) {
			if (pipelineElement.sinkElement != null && pipelineElement.pipeline.IsValid () && TreeFactory.GetActiveInstance() != null) {
		...
		bool DrawAdvancedPanel (StructureGenerator.StructureLevel selectedLevel, SerializedProperty propStructureLevel, bool levelChanged) {
			...
			bool overrideNoise = EditorGUILayout.Toggle (overrideNoiseLabel, _propOverrideNoise.boolValue);
			ShowHelpBox (MSG_OVERRIDE_NOISE);
			...
			if (overrideNoise) {
				// NOISE.
				EditorGUI.BeginChangeCheck ();
				EditorGUILayout.Slider (propStructureLevel.FindPropertyRelative ("noise"), 0f, 1.5f, noiseLabel);
				ShowHelpBox (MSG_NOISE);

				// NOISE SCALE.
				EditorGUILayout.Slider (propStructureLevel.FindPropertyRelative ("noiseScale"), 0f, 1.5f, noiseScaleLabel);
				ShowHelpBox (MSG_NOISE_SCALE);
				if (EditorGUI.EndChangeCheck ()) {
					levelChanged = true;
				}
		...
		private void DrawNodeCanvas () {
			...
			// Resize the canvas element ONLY if the editor width has changed.
			if (screenWidth != Screen.width && Event.current.type == EventType.Repaint) {
				...
				if (structures[i].generatorId == structureLevelId) {
					...
					if (curveEditor.editMode == BezierCurveEditor.EditMode.Add) {
						if (_singleSelectedCurveId == curveEditor.curveId) {
							curveEditor.OnCurveGUI (structures[i].branch.curve, offset + (structures[i].branch.originOffset * scale), isSelected);
		...
		void DrawStructureLevel (StructureGenerator.StructureLevel level, Vector3 offset, float scale = 1) {
			...
			for (int i = 0; i < level.generatedBranches.Count; i++) {
				curveEditor.OnCurveGUI (level.generatedBranches[i].curve, offset + (level.generatedBranches[i].origin * scale));
			}
		}
			...
			Vector3 origin, Vector3 offset, float scale = 1f) 
		{
			...
			if (branch.helperStructureLevelId == structureLevelId) {
				...
				_editStructureLevelId = structureLevelId;
				curveEditor.scale = scale;
				curveEditor.OnCurveGUI (branch.curve, offset + (branch.origin * scale));
			}
		...
		BezierCurveEditor.ControlType OnCheckNodeMoveControls (BezierNode node, int index, System.Guid curveId) {
			...
			return BezierCurveEditor.ControlType.Move;
		}
		#endregion
	}
}
