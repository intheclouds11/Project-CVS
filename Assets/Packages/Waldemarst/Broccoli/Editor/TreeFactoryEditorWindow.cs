...
namespace Broccoli.TreeNodeEditor
{
	...
	public class TreeFactoryEditorWindow : EditorWindow 
	{
		...
		private static GUIContent prefabIncludeMeshesInPrefabLabel = new GUIContent ("Include meshes inside Prefab",
			"If enabled then mesh assets will be included inside the Prefab instead of inside a folder belonging to the Prefab.");
		private static GUIContent prefabIncludeMaterialsInPrefabLabel = new GUIContent ("Include materials inside Prefab",
			"If enabled then material assets will be included inside the Prefab instead of inside a folder belonging to the Prefab.");
		...
		public static void SetupGraphView (TreeFactory treeFactory, TreeFactoryEditorWindow factoryWindow) {
			...
			factoryWindow.pipelineGraph.LoadPipeline (treeFactory.localPipeline, 
				treeFactory.treeFactoryPreferences.graphOffset, 
				/*treeFactory.treeFactoryPreferences.graphZoom*/1f);
			factoryWindow.pipelineGraph.SetHeader (treeFactory.gameObject.name);
			factoryWindow.UpdatePipelineGraphFooter ();
		}
		/// <summary>
		...
		/// <summary>
		/// Event to call when the hierarchy of GameObjects on the Editor changes.
		/// </summary>
		void OnHierarchyChanged () {
			if (treeFactory != null && pipelineGraph != null)
				pipelineGraph.SetHeader (treeFactory.gameObject.name);
		}
		/// <summary>
		...
		private void OnGUI () {
			...
			if (EventType.ValidateCommand == Event.current.type &&
				...
				if (treeFactory != null && pipelineGraph != null && 
					pipelineGraph.lastUndoProcessed != treeFactory.localPipeline.undoControl.undoCount) {
			...
			if (loadedPipeline != null) {
					...
					Broccoli.Manager.MaterialManager.renderPipelineType == Broccoli.Manager.MaterialManager.RenderPipelineType.HDRP)
				{
					...
					for (int i = 0; i < branchMappers.Count; i++) {
						...
						if (branchMapper.diffusionProfileSettings == null)
					...
					for (int i = 0; i < sproutMappers.Count; i++) {
						...
						for (int j = 0; j < sproutMapper.sproutMaps.Count; j++) {
							if (sproutMapper.sproutMaps [j].diffusionProfileSettings == null)
		...
		public bool OnProcessPipeline (Broccoli.Pipe.Pipeline pipeline, 
			Broccoli.Model.BroccoTree tree, 
			int lodIndex,
			PipelineElement referenceElement = null, 
			bool useCache = false, 
			bool forceNewTree = false)
		{
			UpdatePipelineGraphFooter ();
			return true;
		}
		public void UpdatePipelineGraphFooter () {
			if (treeFactory != null && pipelineGraph != null) {
				PipelineElement rootElement = treeFactory.localPipeline.root;
				if (rootElement != null) {
					BranchMeshGeneratorElement branchMeshGeneratorElem = 
						(BranchMeshGeneratorElement)rootElement.GetDownstreamElement (PipelineElement.ClassType.BranchMeshGenerator);
					SproutMeshGeneratorElement sproutMeshGeneratorElem = 
						(SproutMeshGeneratorElement)rootElement.GetDownstreamElement (PipelineElement.ClassType.SproutMeshGenerator);
					if (branchMeshGeneratorElem != null && sproutMeshGeneratorElem != null) {
						int totalVerts = branchMeshGeneratorElem.verticesCount + sproutMeshGeneratorElem.verticesCount;
						int totalTris = branchMeshGeneratorElem.trianglesCount + sproutMeshGeneratorElem.trianglesCount;
						pipelineGraph.SetFooter ($"Verts: {totalVerts}, Tris: {totalTris}");
						return;
					}
				}
			}
