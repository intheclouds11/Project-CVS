...
namespace Broccoli.Utils
{
    ...
    public class TextureUtil {
        ...
        #region Combine
        /// <summary>
        /// Creates a new texture by combining multiple textures at specified normalized rectangles.
        /// </summary>
        /// <param name="width">The width of the output texture.</param>
        /// <param name="height">The height of the output texture.</param>
        /// <param name="textures">A list of source Texture2D objects.</param>
        /// <param name="rects">A list of Rects (with x, y, width, height from 0 to 1) defining where each texture should be placed.</param>
        /// <param name="bgColor">Optional background color for the new texture. Defaults to transparent.</param>
        /// <returns>A new Texture2D containing the combined textures, or null on error.</returns>
        public static Texture2D CombineTextures(int width, int height, List<Texture2D> textures, List<Rect> rects, Color? bgColor = null)
        {
            // 1. Input Validation for dimensions
            if (width <= 0 || height <= 0) {
                Debug.LogError("TextureUtil: CombineTextures requires width and height to be greater than 0.");
                return null;
            }

            // Determine background color
            Color backgroundColor = bgColor.GetValueOrDefault(Color.clear);

            // 2. Create the Destination Texture and fill with background color
            ...
            // 3. Validate lists and iterate through each texture to place it on the canvas
            if (textures != null && rects != null && textures.Count == rects.Count)
            {
                for (int i = 0; i < textures.Count; i++)
                {
                    Texture2D sourceTex = textures[i];
                    Rect targetRect = rects[i];

                    if (sourceTex == null) continue; // Skip null textures

                    // Calculate pixel dimensions for this texture
                    int targetX = Mathf.FloorToInt(targetRect.x * width);
                    int targetY = Mathf.FloorToInt(targetRect.y * height);
                    int targetWidth = Mathf.RoundToInt(targetRect.width * width);
                    int targetHeight = Mathf.RoundToInt(targetRect.height * height);

                    if (targetWidth > 0 && targetHeight > 0) {
                        // Ensure the source texture is readable
                        Texture2D readableSrcTex = GetReadableTexture(sourceTex);
                        
                        // Create a temporary texture to hold the scaled version.
                        Texture2D scaledTex = new Texture2D(readableSrcTex.width, readableSrcTex.height);
                        scaledTex.SetPixels(readableSrcTex.GetPixels());
                        scaledTex.Apply();

                        // Use the existing BilinearScale method to resize it to target dimensions.
                        BilinearScale(scaledTex, targetWidth, targetHeight);
                        
                        // Copy the pixels from the scaled texture to the combined texture.
                        combinedTexture.SetPixels(targetX, targetY, targetWidth, targetHeight, scaledTex.GetPixels());

                        // Clean up temporary objects
                        if (readableSrcTex != sourceTex) {
                            // If we created a temporary readable copy, destroy it.
                            UnityEngine.Object.DestroyImmediate(readableSrcTex);
                        }
        ...
        /// <summary>
        /// Creates a readable copy of a texture if it is not already readable.
        /// </summary>
        /// <param name="texture">The source texture.</param>
        /// <returns>A readable Texture2D. The returned texture is a new instance and should be destroyed if the original was not readable.</returns>
        private static Texture2D GetReadableTexture(Texture2D texture) {
            ...
            // Blit the non-readable texture to the RenderTexture
            Graphics.Blit(texture, renderTex);

            // Backup the currently active RenderTexture
            RenderTexture previous = RenderTexture.active;

            // Set the new RenderTexture as the active one
            RenderTexture.active = renderTex;

            // Create a new readable Texture2D and read the pixels from the active RenderTexture
            Texture2D readableText = new Texture2D(texture.width, texture.height);
            readableText.ReadPixels(new Rect(0, 0, renderTex.width, renderTex.height), 0, 0);
            readableText.Apply();

            // Restore the original active RenderTexture and release the temporary one
        ...
        public static Texture2D SaveTextureToFile (Texture2D texture, string filename, bool configureAsset = false, bool applyGammaCorrection = false)
        {
            #if UNITY_EDITOR
            if (texture == null)
            {
			...
			// --- CORRECTION FOR LINEAR TO GAMMA COLOR SPACE ---
            Texture2D textureToSave;
            if (QualitySettings.activeColorSpace == ColorSpace.Linear && applyGammaCorrection) {
                // Create a temporary texture to hold gamma-corrected colors
                textureToSave = new Texture2D(texture.width, texture.height, TextureFormat.ARGB32, false, false);
                Color[] pixels = texture.GetPixels();
                Color[] gammaPixels = new Color[pixels.Length];
                for (int i = 0; i < pixels.Length; i++) {
                    gammaPixels[i] = pixels[i].gamma; // Convert each pixel from Linear to Gamma
                }
                textureToSave.SetPixels(gammaPixels);
                textureToSave.Apply();
