...
namespace Broccoli.Manager
{
	...
	public class MaterialManager {
		...
		public static Shader srpLeavesShader;
		...
		/// <summary>
		/// Default diffusion profile for HDRP. Set by the last Diffusion Profile set on a material.
		/// </summary>
		public static ScriptableObject defaultDiffusionProfile = null;
		#endregion
		...
		public static void FindDefaultDiffusionProfile () {
			#if UNITY_EDITOR
			string[] guids = AssetDatabase.FindAssets("t:DiffusionProfileSettings");
        
			foreach (string guid in guids) {
				string path = AssetDatabase.GUIDToAssetPath(guid);
				ScriptableObject diffusionProfile = AssetDatabase.LoadAssetAtPath<ScriptableObject>(path);
				if (diffusionProfile != null) {
					defaultDiffusionProfile = diffusionProfile;
					return;
				}
			}
			#endif
		...
		public void SetLeavesShader (TreeFactoryPreferences.PreferredShader preferredShader, Shader customShader = null) {
			...
			if (preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8 || preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8Compatible) {
				srpLeavesShader = Shader.Find ("Nature/SpeedTree8");
				...
				if (preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8Compatible && customShader != null) {
					...
					#endif
					if (leavesShaderPipelineType == RenderPipelineType.HDRP) {
						leavesShader = Shader.Find ("HDRP/Nature/SpeedTree8");
					} else if (leavesShaderPipelineType == RenderPipelineType.URP) {
						leavesShader = Shader.Find ("Universal Render Pipeline/Nature/SpeedTree8");
					} else {
			...
			if (preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree7 || preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree7Compatible || fallbackToST7) {
				srpLeavesShader = Shader.Find ("Nature/SpeedTree7");
				if (srpLeavesShader == null) {
					srpLeavesShader = Shader.Find ("Nature/SpeedTree");
				}
		...
		public void SetBranchShader (TreeFactoryPreferences.PreferredShader preferredShader, Shader customShader = null) {
			...
			if (preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8 || preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8Compatible) {
				...
				if (preferredShader == TreeFactoryPreferences.PreferredShader.SpeedTree8Compatible && customShader != null) {
					...
					#endif
					if (barkShaderPipelineType == RenderPipelineType.HDRP) {
						barkShader = Shader.Find ("HDRP/Nature/SpeedTree8");
					} else if (barkShaderPipelineType == RenderPipelineType.URP) {
						barkShader = Shader.Find ("Universal Render Pipeline/Nature/SpeedTree8");
					} else {
		...
		public static void SetDiffusionProfile (Material material, ScriptableObject diffusionProfile, float cutoff) {
		...
		public static Material GetLeavesMaterial (bool overrideWithStandardSRP = false) {
			Material m;
			if (overrideWithStandardSRP) {
				m = new Material (srpLeavesShader);
			} else {
				m = new Material (leavesShader);
			}
			return m;
		}
		/// <summary>
