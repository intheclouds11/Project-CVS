...
namespace Broccoli.Manager
{
	...
	public class SproutCompositeManager {
		...
		/// <summary>
		/// VariationDescriptor id to VariationDescriptor instance dictionary.
		/// </summary>
		/// <typeparam name="int">Id of the Variation.</typeparam>
		/// <typeparam name="VariationDescriptor">VariationDescriptor instance.</typeparam>
		/// <returns>Relationship between a Variation and its id.</returns>
		Dictionary<int, VariationDescriptor> _idToVariation = new Dictionary<int, VariationDescriptor> ();
		/// <summary>
		/// Variation id/LOD to mesh dictionary.
		/// </summary>
		/// <typeparam name="int">Variation id.</typeparam>
		/// <typeparam name="Mesh">Merged mesh.</typeparam>
		/// <returns>Relationship between a Variation and its mesh.</returns>
		Dictionary<int, Mesh> _variationIdToMesh = new Dictionary<int, Mesh> ();
		/// <summary>
		/// Polygon id to mesh frame size.
		/// </summary>
		/// <typeparam name="Hash128">Texture hash for the polygon texture.</typeparam>
		/// <typeparam name="Vector2">Size of the frame in mesh space.</typeparam>
		/// <returns>Relationship between a polygon area id and its size on mesh space.</returns>
		Dictionary<Hash128, Vector2> _polygonHashToSize = new Dictionary<Hash128, Vector2> ();
		/// <summary>
		...
		private Material[] _tmpMaterials = new Material[0];
		private int[] _tmpMaterialInstanceID = new int[0];
		#endregion
		...
		public void Clear () {
			...
			// Clear polygon area mesh sizes.
			_polygonHashToSize.Clear();
		...
		/// <summary>
		/// Remove elements belonging to a snapshot.
		/// </summary>
		...
		private void RemovePolygonAreaElements (PolygonArea polyArea) {
			if (_polygonHashToSize.ContainsKey (polyArea.hash)) {
				_polygonHashToSize.Remove (polyArea.hash);
			}
		...
		public int GetCompositeSnapshotLodId (int snapshotId, int lod, int resolution = 0) {
		...
		public bool HasSnapshotMesh (int snapshotId, int lod, int resolution = 0) {
			int compositeSnapLodId = GetCompositeSnapshotLodId (snapshotId, lod, resolution);
		...
		public Mesh GetSnapshotMesh (int snapshotId, int lod, int resolution = 0, bool useCache = true) {
			...
			if (_snapshotIdToMesh.ContainsKey (compositeSnapLodId) && useCache) {
				...
				MeshJob.ApplyMeshGradientUV4 (mesh, mesh.bounds);
		...
		/// </summary>
		/// <param name="snapshotId">Snapshot Id.</param>
		public void RemoveSnapshotMeshes (BranchDescriptor snapshot) {
			int minId = GetCompositeSnapshotLodId (snapshot.id, 0, 0);
			int maxId = GetCompositeSnapshotLodId (snapshot.id + 1, 0, 0);
			int key;
		...
		/// <summary>
		/// Remove elements belonging to a variation.
		/// </summary>
		/// <param name="variation">Variation descriptor.</param>
		/// <returns><c>true</c> if removed.</returns>
		public bool RemoveVariation (VariationDescriptor variation) {
			// Remove meshes.
			RemoveVariationMeshes (variation);
			
			// Remove variations.
			if (_idToVariation.ContainsKey (variation.id))
				_idToVariation.Remove (variation.id);
			return true;
		}
		/// <summary>
		/// Checks if the manager already has polygons for a variation.
		/// </summary>
		/// <param name="id">Id of the variation or variation.</param>
		/// <returns><c>True</c> if the variation exists.</returns>
		public bool HasVariation (int id) {
			return _idToVariation.ContainsKey (id);
		}
		/// <summary>
		/// Adds a variation to be managed.
		/// </summary>
		/// <param name="variation">Variation instance.</param>
		/// <returns><c>True</c> if added.</returns>
		public bool AddVariation (VariationDescriptor variation) {
			if (!_idToVariation.ContainsKey (variation.id)) {
				_idToVariation.Add (variation.id, variation);
				return true;
			}
			return false;
		}
		/// <summary>
		/// Composite id for variation+lod+resolution.
		/// </summary>
		/// <param name="variationId">Variation id.</param>
		/// <param name="lod">LOD.</param>
		/// <param name="resolution">Resolution.</param>
		/// <returns>Composite id.</returns>
		public int GetCompositeVariationLodId (int variationId, int lod, int resolution = 0) {
			return variationId * 1000 + lod * 100 + resolution;
		}
		/// <summary>
		/// <c>True</c> if the mesh exists in this manager.
		/// </summary>
		...
		/// <param name="resolution"></param>
		/// <returns><c>True</c> if mesh exists.</returns>
		public bool HasVariationMesh (int variationId, int lod, int resolution = 0) {
			int compositeSnapLodId = GetCompositeVariationLodId (variationId, lod, resolution);
			return _variationIdToMesh.ContainsKey (compositeSnapLodId);
		}
		/// <summary>
		/// Gets the mesh for a given variation id and LOD.
		/// </summary>
		/// <param name="variation">Variation.</param>
		/// <param name="lod">LOD for the mesh.</param>
		/// <param name="useCache">Use cached mesh.</param>
		/// <returns>Mesh for the variation / LOD.</returns>
		public Mesh GetVariationMesh (VariationDescriptor variation, int lod, int resolution = 0, bool useCache = true) {
			if (lod > maxLod) lod = maxLod;
			int compositeVarLodId = GetCompositeVariationLodId (variation.id, lod, resolution);
			if (_variationIdToMesh.ContainsKey (compositeVarLodId) && _variationIdToMesh[compositeVarLodId] != null && useCache) {
				return _variationIdToMesh [compositeVarLodId];
			} else {
				Mesh mesh = new Mesh();
				if (variation.variationMeshes.Count > 0 && lod < variation.variationMeshes.Count) {
					VariationDescriptor.VariationMesh varMesh = null;
					for (int i = 0; i < variation.variationMeshes.Count; i++) {
						if (variation.variationMeshes [i].lod == lod) {
							varMesh = variation.variationMeshes [i];
							break;
						}
					}
					if (varMesh == null) return mesh;

					mesh.SetVertices (varMesh.vertices);
					mesh.SetNormals (varMesh.normals);
					mesh.SetTangents (varMesh.tangents);
					mesh.SetUVs (0, varMesh.uv0s);
					mesh.SetUVs (1, varMesh.uv1s);
					mesh.SetTriangles (varMesh.tris, 0);
					mesh.RecalculateBounds ();

					// Apply forward and side bending gradient.
					//MeshJob.ApplyMeshGradientUV4 (mesh, mesh.bounds);

					if (_variationIdToMesh.ContainsKey (compositeVarLodId)) {
						UnityEngine.Object.DestroyImmediate (_variationIdToMesh [compositeVarLodId]);
						_variationIdToMesh.Remove (compositeVarLodId);
					}
					_variationIdToMesh.Add (compositeVarLodId, mesh);
				}
				return mesh;
			}
		}
		/// <summary>
		/// Removes all meshes belonging to a variation.
		/// </summary>
		/// <param name="variation">Variation descriptor.</param>
		public void RemoveVariationMeshes (VariationDescriptor variation) {
			int minId = GetCompositeVariationLodId (variation.id, 0, 0);
			int maxId = GetCompositeVariationLodId (variation.id + 1, 0, 0);
			int key;
			var meshEnum = _variationIdToMesh.GetEnumerator ();
			List<int> idToRemove = new List<int> ();
			while (meshEnum.MoveNext ()) {
				key = meshEnum.Current.Key;
				if (key >= minId && key < maxId) {
			...
			for (int i = 0; i < idToRemove.Count; i++) {
				Object.DestroyImmediate (_variationIdToMesh [idToRemove [i]]);
				_variationIdToMesh.Remove (idToRemove [i]);
			}
			_variationIdToMesh = new Dictionary<int, Mesh> (_variationIdToMesh);
		}
		#endregion
		...
		public bool GenerateTextures (PolygonArea polygonArea, BranchDescriptor snapshot, SnapshotProcessor.Fragment fragment, SproutSubfactory sproutFactory) {
			...
			if (_snapshotTree != null/*_snapshotMesh != null*/) {
				...
				// Register mesh frame size.
				if (_polygonHashToSize.ContainsKey (polygonArea.hash)) {
					_polygonHashToSize.Remove (polygonArea.hash);
				}
				_polygonHashToSize.Add (polygonArea.hash, polygonArea.meshSize);
		...
		/// <summary>
		/// Prepares the mesh to reflect includes and excludes by caching the original color.a value to uv6.w.
		/// </summary>
		public void SaveColorAlphaToUV6s () {
			MeshFilter meshFilter = _snapshotTree.obj.GetComponent<MeshFilter> ();
			Mesh _snapshotMesh = meshFilter.sharedMesh;
			List<Vector4> uv6s = new List<Vector4> ();
			_snapshotMesh.GetUVs (6, uv6s);
			Color[] colors = _snapshotMesh.colors;
			Vector4 uv6;
			bool hasUV6s = uv6s.Count == colors.Length;
			if (!hasUV6s) {
				uv6s = new List<Vector4> (new Vector4[colors.Length]);
			}
		...
		public void ReflectIncludeAndExcludesToMesh (BranchDescriptor snapshot, List<System.Guid> includes, List<System.Guid> excludes) {
			...
			List<Vector4> uv6s = new List<Vector4> ();
			_snapshotMesh.GetUVs (4, uv4s);
			_snapshotMesh.GetUVs (6, uv6s);
			Color[] colors = _snapshotMesh.colors;
			if (colors.Length == 0) {
				colors = new Color[uv4s.Count];
			...
			bool hasUV6s = uv6s.Count == colors.Length;
			for (int i = 0; i < uv4s.Count; i++) {
				// Branches to stay shown keep their alpha value.
				if (hasUV6s && shownSnapshotIds.Contains ((int)uv4s [i].x)) {
					colors [i].a = uv6s[i].w;
				} else {
					colors [i].a = 0f;
				}
		...
		/// <summary>
		/// Get the sizes of polygon areas in mesh space.
		/// </summary>
		/// <returns>List of sizes for all the polygon areas in mesh space.</returns>
		public List<Vector2> GetPolygonAreaMeshSizes () {
			List<Vector2> sizes = new List<Vector2> ();
			var enumTex = _polygonHashToSize.GetEnumerator ();
			while (enumTex.MoveNext ()) {
				sizes.Add (enumTex.Current.Value);
			}
			return sizes;
		}
		/// <summary>
		...
		public Vector2 GetPolygonAreaMeshSize (Hash128 hash) {
			if (_polygonHashToSize.ContainsKey (hash)) {
				return _polygonHashToSize [hash];
			}
			return Vector2.zero;
		}
			...
			Color subsurfaceColor, Texture2D albedoTex, Texture2D normalsTex, Texture2D extrasTex, Texture2D subsurfaceTex, bool enableInstancing = false)
		{
			...
			MaterialManager.SetLeavesMaterialProperties (
				m, Color.white, cutoff, glossiness, metallic, subsurface, subsurfaceColor, 
		...
		public bool GenerateMaterials (PolygonArea polygonArea, BranchDescriptor snapshot, bool overrideWithStandardSRP = false) {
			if (_snapshotTree != null /*_snapshotMesh != null*/) {
				Material m;
				if (overrideWithStandardSRP) {
					m = new Material (Shader.Find ("Hidden/Broccoli/SproutLabComposite"));
				} else {
					m = MaterialManager.GetLeavesMaterial ();
				}
				MaterialManager.SetLeavesMaterialProperties (
                	m, Color.white, 1.0f, 0.1f, 0.1f, 0.5f, Color.white, 
		...
		/// <summary>
		/// Gets an array of standard Render Pipeline SpeedTree materials and returns them to
		/// the current detected Render Pipeline version.
		/// </summary>
		/// <param name="baseMaterials">Base SpeedTree leaves materials.</param>
		/// <returns>SpeedTree materials for the detected RenderPipeline.</returns>
		public Material[] GetRPMaterials (Material[] baseMaterials) {
			// Compare base and temp materials to see if they need rebuilding.
			bool needsRebuilding = _tmpMaterials.Length != baseMaterials.Length;
			if (!needsRebuilding) {
				// Check for material ids.
				for (int i = 0; i < baseMaterials.Length; i++) {
					if (baseMaterials[i] == null || baseMaterials[i].GetInstanceID () != _tmpMaterialInstanceID[i]) {
			...
			// Rebuild temp materials if needed.
			if (needsRebuilding) {
				// Destroy existing materials.
				for (int i = 0; i < _tmpMaterials.Length; i++) {
					Object.DestroyImmediate (_tmpMaterials[i]);					
				}
				_tmpMaterials = new Material[baseMaterials.Length];
				_tmpMaterialInstanceID = new int[baseMaterials.Length];
				// Create temp materials.
				for (int i = 0; i < baseMaterials.Length; i++) {
					Material tmpMat = MaterialManager.GetLeavesMaterial ();
					_tmpMaterials[i] = tmpMat;
					_tmpMaterialInstanceID[i] = baseMaterials[i].GetInstanceID ();
				}
			}

			// Copy properties from base to temp materials.
			for (int i = 0; i < baseMaterials.Length; i++) {
				if (baseMaterials[i] != null)
					_tmpMaterials[i].CopyPropertiesFromMaterial (baseMaterials[i]);
			}

			// Return temp materials.
			return _tmpMaterials;
		}
		#endregion
