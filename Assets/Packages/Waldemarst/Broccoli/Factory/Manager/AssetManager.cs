...
namespace Broccoli.Manager
{
	...
	public class AssetManager {
		...
		*/
		...
		public void BeginWithCreatePrefab (GameObject previewTarget, string prefabSavePath, string prefabName, string _prefabFullPath = "") {
			#if UNITY_EDITOR
			...
			if (!string.IsNullOrEmpty (_prefabFullPath)) {
				if (!_prefabFullPath.StartsWith("Assets", System.StringComparison.OrdinalIgnoreCase)) _prefabFullPath = "Assets" + _prefabFullPath;
				this.prefabFullPath = _prefabFullPath;
				// If the Prefab exists, clean all nested objects.
				//GameObject existingPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(_prefabFullPath);
				Object[] allAssets = AssetDatabase.LoadAllAssetsAtPath (_prefabFullPath);
				if (allAssets != null && allAssets.Length > 1) // Only proceed if there are sub-assets (more than just the main asset)
				{
					AssetDatabase.StartAssetEditing(); // Optional: Group operations for efficiency
					try
					{
						// 2. Iterate through them
						foreach (Object asset in allAssets)
						{
							// 3. Check if it's a sub-asset (important to not remove the main asset!)
							if (asset != null && AssetDatabase.IsSubAsset(asset))
							{
								// 4. Remove the specific sub-asset
								AssetDatabase.RemoveObjectFromAsset(asset);
								// Note: RemoveObjectFromAsset handles removing it from the asset file.
								// You might call DestroyImmediate(asset, true) if you also want to remove it from memory immediately,
								// but often it's not strictly necessary just for cleaning the asset file.
							}
						}
					}
					finally // Ensure StopAssetEditing runs even if errors occur
					{
						AssetDatabase.StopAssetEditing();
						AssetDatabase.SaveAssets(); // Write changes to disk
						AssetDatabase.Refresh();    // Update the editor UI
					}
				}
			} else {
			...
			if (prefabIsValid) {
				...
				while (materialsEnumerator.MoveNext ()) {
					...
					if (string.IsNullOrEmpty (AssetDatabase.GetAssetPath (materialPair.Value [0]))) {
						...
						if (TreeFactory.GetActiveInstance ().treeFactoryPreferences.prefabIncludeMaterialsInsidePrefab) {
				...
				// LODs. 
					...
					if (generateBillboard) {
						...
						billboardBuilder.billboardExtraTexturePath = 
							GetPrefabResourceFolder () + "/" + GlobalSettings.prefabTexturesPrefix + "billboard_extra.png";
						billboardBuilder.billboardSubsurfaceTexturePath = 
							GetPrefabResourceFolder () + "/" + GlobalSettings.prefabTexturesPrefix + "billboard_subsurface.png";
						...
						if (TreeFactory.GetActiveInstance ().treeFactoryPreferences.prefabIncludeMeshesInsidePrefab) {
							if (isST8) {
		...
		public void OptimizeOnGroups () {
			...
			while (groupIdToSubmeshIndexEnumerator.MoveNext ()) {
				...
				// Clear previously group submeshes per texture.
		...
		public void OptimizeForAtlas (int atlasMaximumSize = 512) {
			...
			var materialsEnumerator = materials.GetEnumerator ();
			int submeshIndex;
			// Flags to control if an atlas for normal map, extras and subsurface should be created,
			// if any material lacks the required textures, the atlas for these maps won't be generated.
			bool createNormalAtlas = true;
			bool createExtrasAtlas = true;
			bool createSubsurfaceAtlas = true;
			// Temporary textures to be destroyed at the end of this method.
			List<Texture2D> tmpTextures = new List<Texture2D> ();
			Dictionary<Hash128, Texture2D> copiedTextures = new Dictionary<Hash128, Texture2D> ();

			// Iterate all the materials tho add their textures to the atlases.
			while (materialsEnumerator.MoveNext ()) {
				submeshIndex = materialsEnumerator.Current.Key;
				if (materialParameters.ContainsKey (submeshIndex)) {
					MaterialParams materialParams = materialParameters [submeshIndex];
					// If the texture material is set to be part of an atlas.
					if (materialParams.useInAtlas) {
						Material material = materials [submeshIndex][0];
						// If the texture on the material needs cropping before getting added to the atlas.
						if (materialParams.needsCrop && submeshToArea [submeshIndex] != null) {
							...
							// ALBEDO MAP: crop and register texture.
							Texture2D tmpAlbedoTex;
							if (copiedTextures.ContainsKey (sproutArea.texture.imageContentsHash))
								tmpAlbedoTex = copiedTextures[sproutArea.texture.imageContentsHash];
							else {
								tmpAlbedoTex = textureManager.GetCopy (sproutArea.texture);
								copiedTextures.Add (sproutArea.texture.imageContentsHash, tmpAlbedoTex);
							}
							tmpTextures.Add (tmpAlbedoTex);
							Texture2D texture = TextureUtil.CropTextureRelative (tmpAlbedoTex,
								                    sproutArea.x, 
								                    sproutArea.y, 
								                    sproutArea.width, 
								                    sproutArea.height);
							Texture2D reTex = new Texture2D (1,1, TextureFormat.ARGB32, true, false);
							TexturePacker.PackTextures (out reTex, new Texture2D[1]{texture}, TexturePacker.PackMode.ScaleEven,
								new TexturePacker.TexturePackParams (0, TexturePacker.PixelSize.Px1024, Color.clear, false));
							textureManager.AddTexture (submeshIndex.ToString (), texture);
							textureManager.RegisterTextureToAtlas (submeshIndex.ToString());

							// NORMAL MAP: conditional crop and register texture.
							if (sproutArea.normalMap != null && createNormalAtlas) {
								//Texture2D tmpNormalTex = textureManager.GetCopy (sproutArea.normalMap);
								Texture2D tmpNormalTex;
								if (copiedTextures.ContainsKey (sproutArea.normalMap.imageContentsHash))
									tmpNormalTex = copiedTextures[sproutArea.normalMap.imageContentsHash];
								else {
									tmpNormalTex = textureManager.GetNormalToRGBA (sproutArea.normalMap);
									copiedTextures.Add (sproutArea.normalMap.imageContentsHash, tmpNormalTex);
								}
								tmpTextures.Add (tmpNormalTex);
								Texture2D normalTexture = TextureUtil.CropTextureRelative (tmpNormalTex,
														sproutArea.x, 
														sproutArea.y, 
														sproutArea.width, 
														sproutArea.height);
								...
								createNormalAtlas = false;
							}

							// EXTRA MAP: conditional crop and register texture.
							if (sproutArea.extraMap != null && createExtrasAtlas) {
								Texture2D tmpExtrasTex;
								if (copiedTextures.ContainsKey (sproutArea.extraMap.imageContentsHash))
									tmpExtrasTex = copiedTextures[sproutArea.extraMap.imageContentsHash];
								else {
									tmpExtrasTex = textureManager.GetCopy (sproutArea.extraMap);
									copiedTextures.Add (sproutArea.extraMap.imageContentsHash, tmpExtrasTex);
								}
								tmpTextures.Add (tmpExtrasTex);
								Texture2D extraTexture = TextureUtil.CropTextureRelative (tmpExtrasTex,
														sproutArea.x, 
														sproutArea.y, 
														sproutArea.width, 
														sproutArea.height);
								textureManager.AddTexture (submeshIndex.ToString () + "e", extraTexture);
								textureManager.RegisterTextureToAtlas (submeshIndex.ToString() + "e", "extras_atlas");
							} else {
								createExtrasAtlas = false;
							}

							// EXTRA MAP: conditional crop and register texture.
							if (sproutArea.subsurfaceMap != null && createSubsurfaceAtlas) {
								Texture2D tmpSubsurfaceTex;
								if (copiedTextures.ContainsKey (sproutArea.subsurfaceMap.imageContentsHash))
									tmpSubsurfaceTex = copiedTextures[sproutArea.subsurfaceMap.imageContentsHash];
								else {
									tmpSubsurfaceTex = textureManager.GetCopy (sproutArea.subsurfaceMap);
									copiedTextures.Add (sproutArea.subsurfaceMap.imageContentsHash, tmpSubsurfaceTex);
								}
								tmpTextures.Add (tmpSubsurfaceTex);
								Texture2D subsurfaceTexture = TextureUtil.CropTextureRelative (tmpSubsurfaceTex,
														sproutArea.x, 
														sproutArea.y, 
														sproutArea.width, 
														sproutArea.height);
								textureManager.AddTexture (submeshIndex.ToString () + "s", subsurfaceTexture);
								textureManager.RegisterTextureToAtlas (submeshIndex.ToString() + "s", "subsurface_atlas");
							} else {
								createSubsurfaceAtlas = false;
							}

							rectToSubmeshIndex.Add (i, submeshIndex);
							i++;
						} 
						// or the texture will be added to the atlas as it is.
						else {
							Texture2D albedoTexture = textureManager.GetMainTexture (material);
							if (albedoTexture != null) {
								// ALBEDO MAP: register to atlas.
								textureManager.AddTexture (submeshIndex.ToString (), albedoTexture);
								textureManager.RegisterTextureToAtlas (submeshIndex.ToString ());

								// NORMAL MAP: register to atlas.
								Texture2D normalTexture = textureManager.GetNormalTexture (material);
								if (normalTexture != null && createNormalAtlas) {
									textureManager.AddTexture (submeshIndex.ToString () + "n", normalTexture);
									textureManager.RegisterTextureToAtlas (submeshIndex.ToString () + "n", "normal_atlas");
								} else {
									createNormalAtlas = false;
								...
								// EXTRAS MAP: register to atlas.
								Texture2D extrasTexture = textureManager.GetExtrasTexture (material);
								if (extrasTexture != null && createExtrasAtlas) {
									textureManager.AddTexture (submeshIndex.ToString () + "e", extrasTexture);
									textureManager.RegisterTextureToAtlas (submeshIndex.ToString () + "e", "extras_atlas");
								} else {
									createExtrasAtlas = false;
								}

								// SUBSURFACE MAP: register to atlas.
								Texture2D subsurfaceTexture = textureManager.GetSubsurfaceTexture (material);
								if (subsurfaceTexture != null && createSubsurfaceAtlas) {
									textureManager.AddTexture (submeshIndex.ToString () + "s", subsurfaceTexture);
									textureManager.RegisterTextureToAtlas (submeshIndex.ToString () + "s", "subsurface_atlas");
								} else {
									createSubsurfaceAtlas = false;
			...
			// Deregister incomplete atlases.
			if (!createNormalAtlas && textureManager.HasAtlas ("normal_atlas")) {
				Debug.LogWarning ("Some textures are not set to create a Normal Map Atlas. Skipped.");
				textureManager.DeregisterAtlas ("normal_atlas");
			}
			if (!createExtrasAtlas && textureManager.HasAtlas ("extras_atlas")) {
				Debug.LogWarning ("Some textures are not set to create a Extras Map Atlas. Skipped.");
				textureManager.DeregisterAtlas ("extras_atlas");
			}
			if (!createSubsurfaceAtlas && textureManager.HasAtlas ("subsurface_atlas")) {
				Debug.LogWarning ("Some textures are not set to create a Subsurface Map Atlas. Skipped.");
				textureManager.DeregisterAtlas ("subsurface_atlas");
			}

			// Set atlases settings.
			if (textureManager.HasAtlas ("atlas")) {
				textureManager.SetAtlasSettings ("atlas", new TextureManager.TextureSettings (true, false, Color.clear, true));
			}
			if (textureManager.HasAtlas ("normal_atlas")) {
				textureManager.SetAtlasSettings ("normal_atlas", new TextureManager.TextureSettings (false, true, new Color (0.5f, 0.5f, 1f, 1f)));
			}
			if (textureManager.HasAtlas ("extras_atlas")) {
				textureManager.SetAtlasSettings ("extras_atlas", new TextureManager.TextureSettings (false, false, new Color (0f, 0f, 1f, 1f)));
			}
			if (textureManager.HasAtlas ("subsurface_atlas")) {
				textureManager.SetAtlasSettings ("subsurface_atlas", new TextureManager.TextureSettings (false, false, new Color (0f, 0f, 0f, 1f)));
			}

			// Remove tmp textures used to register items for atlases.
			for (int j = 0; j < tmpTextures.Count; j++) {
				GameObject.DestroyImmediate (tmpTextures[j]);
			}
			var copiedTexEnum = copiedTextures.GetEnumerator ();
			while (copiedTexEnum.MoveNext ()) {
				GameObject.DestroyImmediate (copiedTexEnum.Current.Value);
			}
			...
			if (textureManager.GetTextureCount() > 0) {
				...
				Texture2D extrasAtlasTexture = 
					UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D> (textureManager.GetAtlasAssetPath ("extras_atlas"));
				Texture2D subsurfaceAtlasTexture = 
					UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D> (textureManager.GetAtlasAssetPath ("subsurface_atlas"));
				...
				if (!string.IsNullOrEmpty (textureManager.GetAtlasAssetPath ())) {
					...
					while (rectToSubmeshIndexEnumerator.MoveNext ()) {
						...
						if (normalAtlasTexture != null) {
							...
							materials [submeshIndex] [0].SetTexture ("_BumpMap", normalAtlasTexture);
						}
						// Set extras texture to atlas.
						if (extrasAtlasTexture != null) {
							materials [submeshIndex] [0].SetTexture ("_ExtraTex", extrasAtlasTexture);
						}
						// Set subsurface texture to atlas.
						if (subsurfaceAtlasTexture != null) {
							materials [submeshIndex] [0].SetTexture ("_SubsurfaceTex", subsurfaceAtlasTexture);
						}
						var assetMeshesEnumerator = assetMeshes.GetEnumerator ();
						int meshPass;
						while (assetMeshesEnumerator.MoveNext ()) {
