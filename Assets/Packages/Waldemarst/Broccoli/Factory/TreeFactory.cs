...
namespace Broccoli.Factory
{
	...
	public class TreeFactory : MonoBehaviour {
		...
		/// <summary>
		/// Delegate for Pipeline processing.
		/// </summary>
		/// <param name="pipeline">Broccoli Pipeline instance being processed.</param>
		/// <param name="tree">BroccoTree instance.</param>
		/// <param name="lodIndex">LOD level being processed.</param>
		/// <param name="referenceElement">Element from the pipeline that triggered the processing.</param>
		/// <param name="useCache"><c>True</c> if the factory is using cached data.</param>
		/// <param name="forceNewTree"><c>True</c> if a new BroccoTree should be created.</param>
		/// <returns><c>True</c> if the process progresses.</returns>
		...
		/// <summary>
		/// Delegate for Prefab processing.
		/// </summary>
		/// <param name="pipeline">Broccoli instance being processed.</param>
		/// <param name="tree">BroccoTree instance.</param>
		/// <param name="prefab">GameObject of the Prefab.</param>
		/// <param name="prefabName">Name for the prefab.</param>
		/// <param name="path">Path to save the prefab.</param>
		/// <param name="resourcesPath">Path to the folder containing the prefab resources (meshes, textures and materials).</param>
		/// <param name="assetInfos">List of LOD assets contained in the prefab.</param>
		/// <returns><c>True</c> if the process progresses.</returns>
		public delegate bool ProcessPrefabDelegate (
			Broccoli.Pipe.Pipeline pipeline, 
			BroccoTree tree,
			GameObject prefab,
			string prefabName,
			string path,
		...
		/// <summary>
		/// Event called when a Pipeline has been processed by this TreeFactory.
		/// </summary>
		public ProcessPipelineDelegate onProcessPipeline;
		/// <summary>
		/// Event called before a Pipeline gets processed by this TreeFactory to produce a preview tree.
		/// </summary>
		public ProcessPipelineDelegate onBeforeProcessPipelinePreview;
		/// <summary>
		/// Event called when a Pipeline has been processed by this TreeFactory to produce a preview tree.
		/// </summary>
		public ProcessPipelineDelegate onProcessPipelinePreview;
		/// <summary>
		/// Event called before begining the creation of a prefab.
		/// </summary>
		...
		/// This controls the resolution of brezier curves for branches.
		/// </summary>
		public float curveStepsPerUnit = 20f;
		#endregion
		...
		*/
		...
		/// <summary>
		/// Saves this factory instance id.
		/// Use to check factories created as duplicates of an original factory.
		/// </summary>
		[SerializeField]
		int _instanceId = 0; 
		#endregion
		...
		void Awake() {
			#if UNITY_EDITOR
			// Check for object duplication on newly created objects.
			if (_instanceId != GetInstanceID ()) {
				// If the instance is new, just created.
				if (_instanceId == 0) {
					// Save the instance id.
					_instanceId = GetInstanceID();
				}
				// If the instance is a duplicate.
				else {
					_instanceId = GetInstanceID();
					// The instance is a duplicate.
					if (_instanceId < 0) {
						// If a pipeline is found, we clone it to have a different instance.
						if (localPipeline != null) {
							Pipeline pipelineClone = localPipeline.Clone ();
							TreeFactoryPreferences prefencesClone = treeFactoryPreferences.Clone ();
							...
							LoadPipeline (pipelineClone, "", true, true);
						} 
					}
				}
			}
				...
				if (!previewTreeTransform) {
					...
					if (GlobalSettings.broccoTreeControllerVersion == GlobalSettings.BROCCO_TREE_CONTROLLER_V1) {
						...
						if (windEffectElement && windEffectElement.isActive) {
							...
							broccoTreeController.sproutSway = windEffectElement.minSprout1Sway;
						}
					} else {
						_previewTree.obj.SetActive (false);
						BroccoTreeController2 broccoTreeController = _previewTree.obj.AddComponent<Broccoli.Controller.BroccoTreeController2> ();
						broccoTreeController.localShaderType = (BroccoTreeController2.ShaderType)MaterialManager.leavesShaderType;

						// Set Wind
						WindEffectElement windEffectElement= (WindEffectElement)localPipeline.GetElement (PipelineElement.ClassType.WindEffect, true);
						if (windEffectElement && windEffectElement.isActive) {
							broccoTreeController.windInstance = BroccoTreeController2.WindInstance.Local;
							broccoTreeController.trunkBending = windEffectElement.trunkBending;
						}

						_previewTree.obj.SetActive (true);
					}
		...
		public void DestroyPreviewTree () {
			if (_previewTree != null) {
				...
				if (_previewTree != null && _previewTree.obj != null) {
					...
					DestroyImmediate (_previewTree.obj.GetComponent<BroccoTreeController2>(), true);
		...
		public bool HasEmptyPipeline () {
			if (_localPipeline == null) {
				return false;
			}
			return _localPipeline.IsEmpty ();
		}
		/// <summary>
		...
		public void LoadPipeline (Pipeline pipeline, string filePath, bool buildPreview, bool useLastSeed = false) {
			...
			if (buildPreview) {
				...
				ProcessPipeline (_localPipeline, _previewTree,
					treeFactoryPreferences.previewLODIndex, null, useLastSeed);
			}
		}
		/// <summary>
			...
			bool forceNewTree = false)
		{
					...
					do {
						...
						if (pipelineElement != null) {
							if (pipelineElement.positionWeight >= PipelineElement.meshGeneratorWeight && !isStructureComplete) {
								...
								OnStructureComplete (pipeline, tree, processControl);
								OnBeforeMeshStart (pipeline, tree, processControl);
							}
							if (pipelineElement.positionWeight >= PipelineElement.mapperWeight && !isMeshComplete) {
								isMeshComplete = true;
								OnMeshComplete (pipeline, tree, processControl);
								OnBeforeMappingStart (pipeline, tree, processControl);
							}
							if (pipelineElement.positionWeight >= PipelineElement.effectWeight && !isMappingComplete) {
								isMappingComplete = true;
								OnMappingComplete (pipeline, tree, processControl);
							}
						}
			...
			if (GlobalSettings.broccoTreeControllerVersion == GlobalSettings.BROCCO_TREE_CONTROLLER_V1) {
				...
				BroccoTreeController2 broccoTreeController = _previewTree.obj.GetComponent<Broccoli.Controller.BroccoTreeController2> ();
				if (broccoTreeController == null) {
					broccoTreeController = _previewTree.obj.AddComponent<Broccoli.Controller.BroccoTreeController2> ();
					BroccoTreeController oldBroccoTreeController = _previewTree.obj.GetComponent<Broccoli.Controller.BroccoTreeController> ();
					DestroyImmediate (oldBroccoTreeController);
				}
				if (broccoTreeController != null) {
					broccoTreeController.windInstance = BroccoTreeController2.WindInstance.Local;
					broccoTreeController.localShaderType = (BroccoTreeController2.ShaderType)MaterialManager.leavesShaderType;
					WindEffectElement windEffectElement= (WindEffectElement)localPipeline.GetElement (PipelineElement.ClassType.WindEffect, true);
					if (windEffectElement && windEffectElement.isActive) {
						bool editorWindEnabled = (localPipeline.selectedElement != null && 
							localPipeline.selectedElement.classType == PipelineElement.ClassType.WindEffect && 
							localPipeline.selectedElement.isActive) ||
							(windEffectElement != null && windEffectElement.isActive && windEffectElement.previewWindAlways);
						broccoTreeController.editorWindEnabled = editorWindEnabled;
					} else {
						broccoTreeController.editorWindEnabled = false;
					}
				}
			}
		...
		/// <param name="tree">Tree.</param>
		public void OnStructureComplete (Pipeline pipeline, BroccoTree tree, TreeFactoryProcessControl processControl) {
			if (pipeline.IsValid () && pipeline.root != null) {
				// Set the branch curve resolutions steps based on the LOD set to build the mesh.
				BranchMeshGeneratorElement branchMeshGeneratorElem = 
					(BranchMeshGeneratorElement)pipeline.root.GetDownstreamElement (PipelineElement.ClassType.BranchMeshGenerator);
				if (branchMeshGeneratorElem != null && branchMeshGeneratorElem.isActive) {
					LODDef lod = treeFactoryPreferences.GetLOD (processControl.lodIndex);
					float minBranchAngleTolerance = Mathf.Min (lod.branchAngleToleranceAtBase, lod.branchAngleToleranceAtTop);
					float angleToleranceLerp = Mathf.InverseLerp (10f, 80f, minBranchAngleTolerance);
					curveStepsPerUnit =
						Mathf.Lerp (branchMeshGeneratorElem.maxCurveStepsPerUnit, branchMeshGeneratorElem.minCurveStepsPerUnit, angleToleranceLerp);
				}
			}

			tree.UpdateResolution ((int)curveStepsPerUnit);
			tree.Update ();
			tree.UpdateFollowUps ();
			tree.CalculateSprouts ();
		}
		/// <summary>
		/// Raises the mesh build begin event.
		/// </summary>
		/// <param name="pipeline">Pipeline.</param>
		/// <param name="tree">Tree.</param>
		public void OnBeforeMeshStart (Pipeline pipeline, BroccoTree tree, TreeFactoryProcessControl processControl) {
		}
		/// <summary>
		...
		/// <param name="pipeline">Pipeline.</param>
		/// <param name="tree">Tree.</param>
		public void OnMeshComplete (Pipeline pipeline, BroccoTree tree, TreeFactoryProcessControl processControl) {
		}
		/// <summary>
		/// Raises the mapping build begin event.
		/// </summary>
		/// <param name="pipeline">Pipeline.</param>
		/// <param name="tree">Tree.</param>
		public void OnBeforeMappingStart (Pipeline pipeline, BroccoTree tree, TreeFactoryProcessControl processControl) {
		}
		/// <summary>
		...
		/// <param name="pipeline">Pipeline.</param>
		/// <param name="tree">Tree.</param>
		public void OnMappingComplete (Pipeline pipeline, BroccoTree tree, TreeFactoryProcessControl processControl) {
		}
		#endregion
		...
		public bool CreatePrefab (string prefabFullPath = "") {
			#if UNITY_EDITOR
			...
			if (localPipeline != null && ValidatePipeline (localPipeline)) {
        		...
        		if (System.IO.Directory.Exists(System.IO.Path.GetDirectoryName(Application.dataPath + treeFactoryPreferences.prefabSavePath))) {
					prefabSavePath = "Assets" + treeFactoryPreferences.prefabSavePath;
				} else {
					prefabSavePath = "Assets";
				}

				string prefabSuffix = Broccoli.Utils.FileUtils.GetNumericSuffix (prefabSavePath,
					treeFactoryPreferences.prefabSavePrefix, "prefab");
				_assetManager.BeginWithCreatePrefab (previewTree.obj,
					prefabSavePath, treeFactoryPreferences.prefabSavePrefix + prefabSuffix, prefabFullPath);
				...
				onBeforeEndPrefabCommit?.Invoke (_localPipeline, _previewTree, null, _assetManager.prefabName,
					_assetManager.prefabFullPath, _assetManager.prefabResourcesPath, _assetManager.assetInfos);
				...
				_assetManager.onLODReady -= OnAssetManagerLODReady;
				if (result) {
					prefabPath = _assetManager.prefabFullPath;
					Debug.Log (string.Format ("Prefab Assset <b>{0}</b> created at <b>{1}</b>\n\tPrefab Resources Folder: {2}",
						_assetManager.prefabName, _assetManager.prefabFullPath, _assetManager.prefabResourcesPath));
					onEndPrefabCommit?.Invoke (_localPipeline, _previewTree, (GameObject)_assetManager.prefab, _assetManager.prefabName,
						_assetManager.prefabFullPath, _assetManager.prefabResourcesPath, _assetManager.assetInfos);
				}
		...
		public int GetAtlasWeight (TextureSize atlasTextureSize) {
			if (atlasTextureSize == TextureSize._128px) {
				...
				return 90;
			} else if (atlasTextureSize  == TextureSize._2048px) {
				return 160;
			} else if (atlasTextureSize  == TextureSize._4096px) {
				return 270;
			} else {
				return 120;
			}
		...
		private void SetMaterials (MeshRenderer meshRenderer, int neededMaterialsCount) {
			...
			if (hasUnassignedMaterials) {
				AddLogWarn (MSG_MATERIALS_UNASSIGNED);
			}
			if (hasUnassignedMesh) {
				AddLogWarn (MSG_MESHDATA_UNASSIGNED);
			}
		...
		public void LogPipelineState (Pipeline pipeline) {
			if (pipeline.state != Pipeline.State.Valid) {
				...
				case Pipeline.State.Empty:
					AddLogWarn (MSG_PIPELINE_EMPTY);
					break;
				case Pipeline.State.NoSourceElement:
					AddLogError (MSG_PIPELINE_NO_SOURCE);
					break;
				case Pipeline.State.NoSinkElement:
					AddLogError (MSG_PIPELINE_NO_SINK);
					break;
				case Pipeline.State.MultiElement:
					AddLogError (MSG_PIPELINE_MULTI_ELEMENT);
					break;
				case Pipeline.State.MultiplePipelines:
					AddLogError (MSG_PIPELINE_MULTIPLE);
					break;
				case Pipeline.State.InvalidConnection:
					AddLogError (MSG_PIPELINE_INVALID_CONNECTION);
					break;
				}
			}
		}
		/// <summary>
		/// Checks if a message is already in the log messages.
		/// </summary>
		/// <param name="msg">Message to check against.</param>
		/// <returns>True if the log contains the message.</returns>
		private bool LogContainsMsg (string msg) {
			var logEnum = log.GetEnumerator ();
			while (logEnum.MoveNext ()) {
				if (msg.CompareTo (logEnum.Current.message) == 0) {
					return true;
				}
			}
			return false;
