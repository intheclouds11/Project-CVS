...
namespace Broccoli.Component
{
	...
	public class BranchBenderComponent : TreeFactoryComponent {
		...
		/// <summary>
		/// Maximum length from the tree trunk to the top branch.
		/// </summary>
		private float treeMaxBranchLength;
		/// <summary>
		/// Fast noise instance for branches.
		/// </summary>
		private FastNoiseLite branchNoise = null;
		/// <summary>
		/// Fast noise instance for roots.
		/// </summary>
		private FastNoiseLite rootNoise = null;
		private float noiseAtTopFactor = 1f/4f;
		#endregion
			...
			TreeFactoryProcessControl processControl = null) 
		{
			...
			treeMaxBranchLength = tree.GetMaxLength ();
		}
		/// <summary>
			...
			TreeFactoryProcessControl processControl = null) 
		{
			...
			if (processControl != null) {
				...
				if (pipelineElement != null && tree != null && 
					...
					// Get branch mesh generator element to see if noise can be applied.
					bool cancelNoise = false;
					/*
					BranchMeshGeneratorElement branchMeshGeneratorElement =
						(BranchMeshGeneratorElement)branchBenderElement.GetDownstreamElement (PipelineElement.ClassType.BranchMeshGenerator);
					if (branchMeshGeneratorElement != null && 
						branchMeshGeneratorElement.meshMode == BranchMeshGeneratorElement.MeshMode.Shape)
					{
						cancelNoise = true;
					}
					*/
					...
					if (branchBenderElement.applyNoise && !cancelNoise) {
						ApplyNoise (treeFactory.treeFactoryPreferences.factoryScale);
		...
		public override void Unprocess (TreeFactory treeFactory) {
			RemoveBranchNoise ();
			treeFactory.previewTree.UpdatePosition ();
		}
		...
		private void ApplyDirectionalBendingRecursive (BroccoTree.Branch branch) {
			...
			do {
				...
				if (!branch.isTuned) {
					...
					*/
					branch.ApplyDirectionalLength (newDirection, branch.length);
		...
		private void ApplyFollowUpSmoothing () {
			...
			for (int i = 0; i < branches.Count; i++) {
				if (branches[i].followUp != null) {
					...
					Vector3 followUpDirection = branches[i].GetDirectionAtLength (1f);
		...
		#region Branch Noise
		private void ApplyNoise (float treeFactoryScale) {
			if (UseFastNoise()) {
				// Create noise and set seed.
				branchNoise = new FastNoiseLite (pipelineElement.seed);
				rootNoise = new FastNoiseLite (pipelineElement.seed);

				float noiseFrequency = branchBenderElement.GetFastNoiseFrequency (branchBenderElement.noiseType, branchBenderElement.noiseResolution);
				
				FastNoiseLite.NoiseType noiseType = FastNoiseLite.NoiseType.Perlin;
				switch (branchBenderElement.noiseType) {
					case BranchBenderElement.NoiseType.Perlin: noiseType = FastNoiseLite.NoiseType.Perlin; break;
					case BranchBenderElement.NoiseType.Simplex2: noiseType = noiseType = FastNoiseLite.NoiseType.OpenSimplex2; break;
					case BranchBenderElement.NoiseType.Value: noiseType = FastNoiseLite.NoiseType.Value; break;
				}
				branchNoise.SetNoiseType (noiseType);
				branchNoise.SetFrequency (noiseFrequency);
				branchNoise.SetFractalType (FastNoiseLite.FractalType.FBm);
				branchNoise.SetFractalGain (0.25f);
				branchNoise.SetFractalLacunarity (3f);

				rootNoise.SetNoiseType (noiseType);
				rootNoise.SetFrequency (noiseFrequency);
				rootNoise.SetFractalType (FastNoiseLite.FractalType.FBm);
				rootNoise.SetFractalGain (0.25f);
				rootNoise.SetFractalLacunarity (3f);
			}
			for (int i = 0; i < tree.branches.Count; i++) {
				ApplyNoiseRecursive (tree.branches [i], (branchBenderElement.hasRandomNoiseOffset?Random.Range (-1f, 1f):branchBenderElement.noiseOffset));
			}
		}
		private bool UseFastNoise () {
			if (GlobalSettings.experimentalFastNoise && branchBenderElement.noiseType != BranchBenderElement.NoiseType.Basic) {
				return true;
			}
			return false;
		}
		private void ApplyNoiseRecursive (BroccoTree.Branch branch, float lengthOffset) {
			...
			float scaleFactorAtBase = Mathf.InverseLerp (0f, treeMaxBranchLength, branch.GetLengthAtPos (0f, true));
			float scaleFactorAtTop = Mathf.InverseLerp (0f, treeMaxBranchLength, branch.GetLengthAtPos (1f, true));

			float noiseFactorAtFirstNodeFactor = 1f;
			float noiseFactorAtLastNodeFactor = 1f;
			float noiseScaleAtFirstNodeFactor = 1f;
			float noiseScaleAtLastNodeFactor = 1f;
			...
			if (overrideNoiseLevels.Count > 0 && overrideNoiseLevels.ContainsKey (branch.helperStructureLevelId)) {
				...
				noiseFactorAtFirstNodeFactor = structureLevel.noise;
				noiseFactorAtLastNodeFactor = structureLevel.noise;
				noiseScaleAtFirstNodeFactor = structureLevel.noiseScale;
				noiseScaleAtLastNodeFactor = structureLevel.noiseScale;
			}

			if (branch.isRoot) {
				if (UseFastNoise()) {
					branch.curve.noise = rootNoise;
				} else {
					branch.curve.noise = null;
				}
				branch.curve.SetNoise (
					Mathf.Lerp (branchBenderElement.noiseAtRootBase, branchBenderElement.noiseAtRootBottom, factorAtBase) * noiseFactorAtFirstNodeFactor,
					Mathf.Lerp (branchBenderElement.noiseAtRootBase, branchBenderElement.noiseAtRootBottom, factorAtTop) * noiseFactorAtLastNodeFactor,
					Mathf.Lerp (branchBenderElement.noiseScaleAtRootBase, branchBenderElement.noiseScaleAtRootBottom, scaleFactorAtBase) * noiseScaleAtFirstNodeFactor,
					Mathf.Lerp (branchBenderElement.noiseScaleAtRootBase, branchBenderElement.noiseScaleAtRootBottom, scaleFactorAtTop) * noiseScaleAtLastNodeFactor,
					Mathf.Lerp (tree.maxGirth, tree.maxGirth * noiseAtTopFactor, factorAtBase),
					Mathf.Lerp (tree.maxGirth, tree.maxGirth * noiseAtTopFactor, factorAtTop),
					branch.GetLengthAtPos (0f, true), branch.GetLengthAtPos (1f, true),
					false,
					branchBenderElement.noiseResolution,
					branchBenderElement.noiseStrength,
					lengthOffset);
			} else {
				if (UseFastNoise()) {
					branch.curve.noise = branchNoise;
				} else {
					branch.curve.noise = null;
				}
				branch.curve.SetNoise (
					Mathf.Lerp (branchBenderElement.noiseAtBase, branchBenderElement.noiseAtTop, factorAtBase) * noiseFactorAtFirstNodeFactor,
					Mathf.Lerp (branchBenderElement.noiseAtBase, branchBenderElement.noiseAtTop, factorAtTop) * noiseFactorAtLastNodeFactor,
					Mathf.Lerp (branchBenderElement.noiseScaleAtBase, branchBenderElement.noiseScaleAtTop, scaleFactorAtBase) * noiseScaleAtFirstNodeFactor,
					Mathf.Lerp (branchBenderElement.noiseScaleAtBase, branchBenderElement.noiseScaleAtTop, scaleFactorAtTop) * noiseScaleAtLastNodeFactor,
					Mathf.Lerp (tree.maxGirth, tree.maxGirth * noiseAtTopFactor, factorAtBase),
					Mathf.Lerp (tree.maxGirth, tree.maxGirth * noiseAtTopFactor, factorAtTop),
					branch.GetLengthAtPos (0f, true), branch.GetLengthAtPos (1f, true),
					false,
			...
			branch.curve.ProcessAfterCurveChanged ();
			for (int i = 0; i < branch.branches.Count; i++) {
				ApplyNoiseRecursive (branch.branches[i],
					lengthOffset + (branch.curve.length * branch.branches[i].position) + (branch.branches[i].isFollowUp?0f:branch.branches[i].length * Random.Range(0f, 1f)));
			}
		}
		private void RemoveBranchNoise (BroccoTree.Branch branch = null) {
			if (branch == null) {
				for (int i = 0; i < tree.branches.Count; i++) {
					RemoveBranchNoise (tree.branches [i]);
				}
			} else {
				branch.curve.noise = null;
				branch.curve.SetNoise (0f, 0f, 1f, 1f, 1f, 1f, 0f, 0f, false, 4f, 0.2f, 0f);
				branch.curve.ComputeSamples ();
				branch.curve.ProcessAfterCurveChanged ();
				for (int i = 0; i < branch.branches.Count; i++) {
					RemoveBranchNoise (branch.branches [i]);
				}
			}
		}
		#endregion
