...
namespace Broccoli.Component
{
	...
	public class BranchMeshGeneratorComponent : TreeFactoryComponent {
		...
		private void BuildMesh (TreeFactory treeFactory, int lodIndex) {
			...
			meshBuilder.branchAngleToleranceAtBase = lod.branchAngleToleranceAtBase;
			meshBuilder.branchAngleToleranceAtTop = lod.branchAngleToleranceAtTop;
		...
		private void CreateTrunkMeshBuilder (TrunkMeshGeneratorElement trunkMeshGeneratorElement, TreeFactory treeFactory, int lodIndex) {
			...
			for (int i = 0; i < tree.branches.Count; i++) {
				...
				// Set trunk range.
				if (trunkMeshGeneratorElement.integrationMode != TrunkMeshGeneratorElement.IntegrationMode.None) {
					// Create range.
					float range = Mathf.Lerp (
						trunkMeshGeneratorElement.minSpread, 
						trunkMeshGeneratorElement.maxSpread, 
						Random.Range (0f, 1f));
					BranchMeshBuilder.BranchSkinRange branchSkinRange = new BranchMeshBuilder.BranchSkinRange ();
					branchSkinRange.from = 0f;
					branchSkinRange.to = range;
					...
					branchSkinRange.subdivisions = 
						(int)Mathf.Lerp (24f * trunkMeshGeneratorElement.lengthResolutionFactor * range, 
							12f * trunkMeshGeneratorElement.lengthResolutionFactor * range, Mathf.InverseLerp (5, 45, lod.branchAngleToleranceAtBase));
					...
					branchSkin.AddBuilderRange (branchSkinRange);

					// Get trunk component.
					TrunkMeshGeneratorComponent trunkMeshGeneratorComponent = 
						(TrunkMeshGeneratorComponent)treeFactory.componentManager.GetFactoryComponent (trunkMeshGeneratorElement);

					// Register branch values on the trunk mesh builder for simulated roots generation.
					/*
					if (trunkMeshGeneratorElement.integrationMode == TrunkMeshGeneratorElement.IntegrationMode.SimulateRoots) {
						////////////////////////////////
						/// BUILDER TrunkMeshBuilder ///
						////////////////////////////////
						// Set range type as trunk.
						branchSkinRange.builderType = BranchMeshBuilder.BuilderType.Trunk;

						// Get the TrunkMeshBuilder.
						TrunkMeshBuilder trunkMeshBuilder;
						if (!meshBuilder.ContainsMeshBuilder (BranchMeshBuilder.BuilderType.Trunk)) {
							trunkMeshBuilder = new TrunkMeshBuilder ();
							trunkMeshBuilder.branchAngleToleranceAtBase = lod.branchAngleToleranceAtBase;
							trunkMeshBuilder.minPolygonSides = Mathf.RoundToInt (lod.minPolygonSides * trunkMeshGeneratorElement.radialResolutionFactor);
							trunkMeshBuilder.maxPolygonSides = Mathf.RoundToInt (lod.maxPolygonSides * trunkMeshGeneratorElement.radialResolutionFactor);
							meshBuilder.AddMeshBuilder (trunkMeshBuilder);
						} else {
							trunkMeshBuilder = (TrunkMeshBuilder)meshBuilder.GetBranchMeshBuilder (BranchMeshBuilder.BuilderType.Trunk);
						}
						
						// Register Simulated roots trunk.
						trunkMeshGeneratorComponent.RegisterPseudoTrunkMesh (
							trunkMeshBuilder,
							tree.branches [i],
							branchSkin,
							branchSkinRange,
					...
					*/
						/////////////////////////////////
						/// BUILDER CustomMeshBuilder ///
						/////////////////////////////////
						// Set range type as custom.
						branchSkinRange.builderType = BranchMeshBuilder.BuilderType.Custom;

						// Get the CustomMeshBuilder.
						CustomTrunkMeshBuilder customMeshBuilder;
						if (!meshBuilder.ContainsMeshBuilder (BranchMeshBuilder.BuilderType.Custom)) {
							customMeshBuilder = new CustomTrunkMeshBuilder ();
							customMeshBuilder.branchAngleToleranceAtBase = lod.branchAngleToleranceAtBase;
							customMeshBuilder.minPolygonSides = Mathf.RoundToInt (lod.minPolygonSides * trunkMeshGeneratorElement.radialResolutionFactor);
							customMeshBuilder.maxPolygonSides = Mathf.RoundToInt (lod.maxPolygonSides * trunkMeshGeneratorElement.radialResolutionFactor);
							meshBuilder.AddMeshBuilder (customMeshBuilder);
						} else {
							customMeshBuilder = (CustomTrunkMeshBuilder)meshBuilder.GetBranchMeshBuilder (BranchMeshBuilder.BuilderType.Custom);
						}
						
						// Register Simulated roots trunk.
						trunkMeshGeneratorComponent.RegisterCustomTrunkMesh (
							customMeshBuilder,
							tree.branches [i],
							branchSkin,
							branchSkinRange,
