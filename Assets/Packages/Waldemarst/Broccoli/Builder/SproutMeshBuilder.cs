	...
	public class SproutMeshBuilder {
		...
		void PrepareData (BroccoTree tree, int sproutGroupId, SproutMesh sproutMesh, SproutMap.SproutMapArea sproutArea = null, int sproutAreaIndex = -1) {
			...
			if (sproutArea != null && 
				...
				sproutMesh.overridedHeight = planeDef.height;
		...
		public void AssignSproutSubgroups (BroccoTree tree, int sproutGroupId, BranchDescriptorCollection branchCollection, SproutMesh sproutMesh) {
			sproutMesh.meshType = branchCollection.descriptorImplId;

			enabledSubgroups.Clear ();

			// SNAPSHOTS
			if (sproutMesh.meshType == BranchDescriptorCollection.SNAPSHOT_COLLECTION ||
				sproutMesh.meshType == BranchDescriptorCollection.BASE_COLLECTION) {
				if (sproutMesh.branchCollectionItemsEnabled.Count == 0) {
					for (int i = 0; i < branchCollection.snapshots.Count; i++) {
						sproutMesh.branchCollectionItemsEnabled.Add (true);
					}
				}
				for (int i = 0; i < branchCollection.snapshots.Count; i++) {
					if (sproutMesh.branchCollectionItemsEnabled[i])
			...
			// VARIATIONS
			else if (sproutMesh.meshType == BranchDescriptorCollection.VARIATION_COLLECTION) {
				for (int i = 0; i < branchCollection.variations.Count; i++) {
					if (sproutMesh.branchCollectionItemsEnabled[i])
						enabledSubgroups.Add (i);
				}
				sproutMesh.subgroups = enabledSubgroups.ToArray ();
				
				int maxVariationIndex = enabledSubgroups.Count;
				List<BroccoTree.Branch> branches = tree.GetDescendantBranches ();
				bool isBranchFist;
				for (int i = 0; i < branches.Count; i++) {
					isBranchFist = true;
					for (int j = 0; j < branches[i].sprouts.Count; j++) {
						if (branches[i].sprouts[j].groupId == sproutGroupId) {
							...
							isBranchFist = false;
							if (maxVariationIndex > 0) {
								branches[i].sprouts[j].subgroupId = enabledSubgroups [Random.Range (0, maxVariationIndex)];
							} else {
								branches[i].sprouts[j].subgroupId = -1;
							}
						}
		...
		void MeshSprout (BroccoTree.Sprout sprout, BroccoTree.Branch branch, SproutMesh sproutMesh) {
			...
			sprout.meshWidth = scale * planeDef.width * (1f - planeDef.pivotPosW);
			sprout.meshDepth = scale * sproutMesh.depth;
