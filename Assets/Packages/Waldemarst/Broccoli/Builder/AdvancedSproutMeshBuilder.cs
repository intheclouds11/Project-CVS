...
namespace Broccoli.Builder
{
	...
	public class AdvancedSproutMeshBuilder {
		...
		struct SproutJob : IJobParallelFor {
			...
			/// <summary>
			/// Contains the SPROUT_WIND_PATTERN, SPROUT_NOISE_PATTERN, SPROUT_NOISE_FREQUENCY, SPROUT_NOISE_STRENGTH.
			/// SPROUT_NOISE_STRENGTH is normalized 0-1, the actual amount of displacement is obtained by multiplying this value with the SPROUT_NOISE_DISPLACEMENT value.
			/// </summary>
			public NativeArray<Vector4> sWPattern_sNPattern_sNFreq_sNStr;
			/// <summary>
			/// Contains the SPROUT_NOISE_DISPLACEMENT length values.
			/// </summary>
			public NativeArray<float> sNDisplacement;
			/// <summary>
			...
			public void Execute (int i) {
				...
				float spWPattern = sWPattern_sNPattern_sNFreq_sNStr[i].x;
				float snPattern = sWPattern_sNPattern_sNFreq_sNStr[i].y;

				if (snPattern != 0) {
					float snFreq = sWPattern_sNPattern_sNFreq_sNStr[i].z;
					float snStr = sWPattern_sNPattern_sNFreq_sNStr[i].w;
					float snDisp = sNDisplacement[i];
					ApplyNoise (vertexStart, vertexEnd, snPattern, snFreq, snStr, snDisp, spRand);
				}
			...
			private void ApplyNoise (int vertexStart, int vertexEnd, float noisePattern, float noiseFreq, float noiseStr, float noiseDisp, float randomValue) {
				FastNoiseLite fn = FastNoiseLite.instance;
				fn.SetNoiseType (FastNoiseLite.NoiseType.OpenSimplex2S);
				fn.SetFrequency (noiseFreq);
				fn.SetFractalType (FastNoiseLite.FractalType.FBm);
				fn.SetFractalGain (0.2f);
				fn.SetFractalLacunarity (6f);
				Vector3 vert, normal;
				Vector4 tangent;
				float noise, noiseNorm;
				float _noiseStr;
				float offset = 25f * randomValue;
				
				for (int i = vertexStart; i < vertexEnd; i++) {
					vert = vertices [i];
					noise = fn.GetNoise (vert.x + offset, vert.z + offset);
					noiseNorm = noise / 2f + 0.5f;
					if (noisePattern == 1) { // Forward
						_noiseStr = noiseStr * uv2s[i].x;
						vert.y = noise * noiseDisp * _noiseStr;
					} else if (noisePattern == 2) { // Side
						_noiseStr = noiseStr * uv2s[i].y;
						vert.y = noise * noiseDisp * _noiseStr;
					} else if (noisePattern == 3) { // Both
						_noiseStr = noiseStr * uv2s[i].x * uv2s[i].y;
						vert.y = noise * noiseDisp * _noiseStr;
					} else {
						_noiseStr = noiseStr;
						vert.y = noise * noiseStr * noiseDisp;
					}
					vertices[i] = vert;
					fn.GetNormal (vert.x + offset, vert.z + offset, out normal, out tangent);
					_noiseStr = Mathf.Clamp01(_noiseStr) * 0.7f;
					normals[i] = Vector3.Lerp (normals[i], normal, _noiseStr * noiseNorm);
					tangents[i] = tangent;
 				}
			} 
		...
		/// <summary>
		/// Contains the SPROUT_WIND_PATTERN, NOISE_PATTERN, NOISE_FREQUENCY, NOISE_STRENGTH
		/// </summary>
		public List<Vector4> _sproutWPatternNPatternNFreqNStrength = new List<Vector4> ();
		/// <summary>
		/// Contains the SPROUT_NOISE_DISPLACEMENT
		/// </summary>
		public List<float> _sproutNDisplacement = new List<float> ();
		#endregion

		#region Events
		/// <summary>
		/// Delegate to call for an event on a branch relative to its sprouts grouping.
		/// </summary>
		/// <param name="groupId"></param>
		/// <param name="subgroupId"></param>
		/// <param name="parentBranch"></param>
		public delegate void OnBeforeSproutGrouping (int groupId, int subgroupId, BroccoTree.Branch parentBranch);
		/// <summary>
		/// Called before the Sprouts of a branch are assigned subgroups.
		/// </summary>
		public OnBeforeSproutGrouping onBeforeAssignSproutSubgroups;
		#endregion
		...
		/// <param name="lod">Target LOD.</param>
		public void RegisterMesh (Mesh mesh, int meshType, int groupId, int subgroupId = -1, int lod = 0) {
			int meshId = GetGroupSubgroupId (meshType, groupId, subgroupId, lod);
		...
		public int GetGroupSubgroupId (int meshType, int groupId, int subgroupId = -1, int lod = 0) {
			if (subgroupId < 0) subgroupId = -1;
			int meshId = meshType * 1000000 + groupId * 10000 + (subgroupId + 1) * 10 + lod;
			return meshId;
		}
		#endregion
		...
		private void ClearSproutParams () {
			...
			_sproutWPatternNPatternNFreqNStrength.Clear ();
			_sproutNDisplacement.Clear ();
		}
		#endregion
			...
			int lodIndex = 0)
		{
			...
			int meshId = GetGroupSubgroupId (sproutMesh.meshType, groupId, subgroupId, lodIndex);
			if (_idToMesh.ContainsKey (meshId)) {
				...
				SproutJob _sproutJob = new SproutJob () {
					...
					sWPattern_sNPattern_sNFreq_sNStr = new NativeArray<Vector4> (_sproutWPatternNPatternNFreqNStrength.ToArray (), Allocator.TempJob),
					sNDisplacement = new NativeArray<float> (_sproutNDisplacement.ToArray (), Allocator.TempJob),
				...
				_sproutJob.sWPattern_sNPattern_sNFreq_sNStr.Dispose ();
				_sproutJob.sNDisplacement.Dispose ();
		...
		private void AddSprout (int vertexStart, int vertexLength, BroccoTree.Sprout sprout, BroccoTree.Branch branch, SproutMesh sproutMesh) {
			...
			// Noise
			int noisePattern = (GlobalSettings.experimentalFastNoiseSprouts?(int)sproutMesh.noisePattern:0);
			float noiseFreq = 0f;
			float noiseStr = 0f;
			if (sproutMesh.shapeMode == SproutMesh.ShapeMode.GridPlane) {
				if (sproutMesh.noisePattern != SproutMesh.NoisePattern.None) {
					float noisePosFactor;
					if (sproutMesh.noiseDistribution == SproutMesh.NoiseDistribution.Hierarchy) {
						noisePosFactor = sprout.GetPositionFactorHierarchy (sproutMesh.scaleVariance, sproutMesh.scaleCurve);
					} else if (sproutMesh.noiseDistribution == SproutMesh.NoiseDistribution.Branch) {
						noisePosFactor = sprout.GetPositionFactorBranch (sproutMesh.scaleVariance, sproutMesh.scaleCurve);
					} else {
						noisePosFactor = sprout.GetPositionFactorRange (sproutMesh.scaleVariance, sproutMesh.scaleCurve);
					}
					noiseFreq = Mathf.Lerp (sproutMesh.noiseResolutionAtBase, sproutMesh.noiseResolutionAtTop, noisePosFactor);
					noiseStr = Mathf.Lerp (sproutMesh.noiseStrengthAtBase, sproutMesh.noiseStrengthAtTop, noisePosFactor);
				}
			} else {
				noisePattern = 0;
			}
			...
			_sproutWPatternNPatternNFreqNStrength.Add (new Vector4 ((float)sproutMesh.windPattern, (float)noisePattern, noiseFreq, noiseStr));
			_sproutNDisplacement.Add ((sprout.meshHeight + sprout.meshWidth) / 2f);
		}
		#endregion
		...
		public void AssignSproutSubgroups (BroccoTree tree, int sproutGroupId, SproutMap sproutMap, SproutMesh sproutMesh) {
			sproutMesh.meshType = 0;
			...
			for (int i = 0; i < branches.Count; i++) {
				...
				for (int j = 0; j < branches[i].sprouts.Count; j++) {
					if (branches[i].sprouts[j].groupId == sproutGroupId) {
						if (isBranchFist) {
							onBeforeAssignSproutSubgroups?.Invoke (sproutGroupId, -1, branches[i]);
						}
						isBranchFist = false;
						if (maxAreaIndex > 0) {
