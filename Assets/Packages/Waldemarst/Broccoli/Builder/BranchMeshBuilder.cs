...
namespace Broccoli.Builder {
	...
	public class BranchMeshBuilder {
		...
		struct BranchJob : IJobParallelFor {
			...
			/// <summary>
			/// Contains the segment NORMALS when using shapes (optional),
			/// the normals are unrotated,
			/// </summary>
			[NativeDisableParallelForRestriction]
			public NativeArray<Vector4> aShapeNormals;
			#endregion	
			...
			/// </summary>
			[NativeDisableParallelForRestriction]
			public NativeArray<Vector4> uv0s;
			...
			/// </summary>
			[NativeDisableParallelForRestriction]
			public NativeArray<Vector4> uv1s;
					...
					*/
						...
						!isFirstSegment)
					{
						...
						if (prevSegmentSideCount == currSegmentSideCount) {
							...
							float aSegmentValue = uv0s [aSegmentIndex].x; // A segment index radial value.
							float bSegmentValue = uv0s [bSegmentIndex].x; // B segment index radial value.
							float halfA = (aSegmentValue + uv0s [aSegmentIndex + 1].x) /2f; // half value between segment A index and segment A index + 1.
							while (bSegmentValue < 1) {
								if (bSegmentValue < halfA) {
									...
									trisIndex += 3;
									bSegmentIndex++;
									bSegmentValue = uv0s [bSegmentIndex].x;
								} else {
									// Segment A
									triangles [trisStartAt + trisIndex] = bSegmentIndex;
									if (inverse) {
									...
									trisIndex += 3;
									aSegmentIndex++;
									aSegmentValue = uv0s [aSegmentIndex].x;
									if (aSegmentValue < 1f) {
										halfA = (aSegmentValue + uv0s [aSegmentIndex + 1].x) /2f;
									} else {
					...
					for (int i = 0; i <= polygonSides; i++) {
						...
						if (!useHardNormals) {
							if (segmentType == BranchMeshBuilder.BranchSkin.SEGMENT_TYPE_END_CAP) {
								...
								// Normal provided by shaper.
								if (shapeStartIndex >= 0) {
									vertexNormal = aShapeNormals [shapeStartIndex + i];
						...
						/// z: radial position.
						/// w: girth.
						uv0s [startIndex + indexPos] = new Vector4 (
							radialPosition, 
						...
						/// x, y, z: direction.
						/// w: unallocated.
						uv6s [startIndex + indexPos] = 
							new Vector4 (direction_posAtSkin.x, 
								direction_posAtSkin.y, 
								direction_posAtSkin.z);

						/// UV8 ch7
						/// x: radial position.
						/// y: accumulated length.
						/// z: girth.
						/// w: branchskin length position.
						uv7s [startIndex + indexPos] = 
							new Vector4 (radialPosition,
								bDir_phasePosition_accumLength_status.z,
								up_girth.w,
								direction_posAtSkin.w);
		...
		public class BranchSkin {
			...
			/// <summary>
			/// How many shape normals contains this branch skin (if 0 the normals are calculated).
			/// </summary>
			public int totalShapeNormals {
				get { return _shape_normals.Count; }
			}
			/// <summary>
			...
			/// <summary>
			/// Keeps the SHAPE_NORMAL and RADIAL_POSITION for all the shape segments (could be empty, then the normal for shapes are calculated).
			/// </summary>
			protected List<Vector3> _shape_normals = new List<Vector3> ();
			/// <summary>
			...
			public List<Vector3> shapeNormals {
				get { return _shape_normals; }
			}
			...
			/// <param name="normal">Normal on the segment..</param>
			/// <param name="radialPosition">Radial position (0-1).</param>
			public void AddShapeSegmentVertex (Vector3 vertex, Vector3 normal, float radialPosition) {
				_shape_vertices_positions.Add (new Vector4 (vertex.x, vertex.y, vertex.z, radialPosition));
				_shape_normals.Add (normal);
			}
			/// <summary>
			...
			/// <returns>Center point at length.</returns>
			...
			/// <summary>
			/// Gets the surface point at a position (0 to 1) on this BranchSkin instance.
			/// </summary>
			/// <param name="positionAtSkin">Position on the branch skin.</param>
			/// <param name="firstBranch">First branch on the BranchSkin.</param>
			/// <param name="rollAngle">Roll angle in radians applied to the branch normal.</param>
			/// <returns>Surface point at position.</returns>
			public Vector3 GetSurfacePointAtPosition (float positionAtSkin, BroccoTree.Branch firstBranch, float rollAngle = 0f) {
				Vector3 surfacePoint = Vector3.zero;
				BroccoTree.Branch targetBranch = null;
				float targetBranchPosition = TranslateToPositionAtBranch (positionAtSkin, firstBranch, out targetBranch);
				surfacePoint = targetBranch.GetSurfacePointAt (targetBranchPosition, rollAngle);
				return surfacePoint;
			}
			/// <summary>
			/// Gets the surface point at a length position on this BranchSkin instance.
			/// </summary>
			/// <param name="positionAtSkin">Position on the branch skin.</param>
			/// <param name="firstBranch">First branch on the BranchSkin.</param>
			/// <param name="rollAngle">Roll angle in radians applied to the branch normal.</param>
			/// <returns>Surface point at length.</returns>
			public Vector3 GetSurfacePointAtLength (float lengthAtSkin, BroccoTree.Branch firstBranch, float rollAngle = 0f) {
				return GetSurfacePointAtPosition (lengthAtSkin / length, firstBranch, rollAngle);
			}
			/// <summary>
			/// Gets the surface normal at a position (0 to 1) on this BranchSkin instance.
			/// </summary>
			/// <param name="positionAtSkin">Position on the branch skin.</param>
			/// <param name="firstBranch">First branch on the BranchSkin.</param>
			/// <param name="rollAngle">Roll angle in radians applied to the branch normal.</param>
			/// <returns>Surface normal at position.</returns>
			public Vector3 GetSurfaceNormalAtPosition (float positionAtSkin, BroccoTree.Branch firstBranch, float rollAngle = 0f) {
				Vector3 surfaceNormal = Vector3.forward;
				BroccoTree.Branch targetBranch = null;
				float targetBranchPosition = TranslateToPositionAtBranch (positionAtSkin, firstBranch, out targetBranch);
				surfaceNormal = targetBranch.GetSurfaceNormalAt (targetBranchPosition, rollAngle);
			...
			public Vector3 GetNormalAtPosition (float positionAtSkin, BroccoTree.Branch firstBranch, float rollAngle = 0f) {
				Vector3 normal = Vector3.zero;
				BroccoTree.Branch targetBranch = null;
				float targetBranchPosition = TranslateToPositionAtBranch (positionAtSkin, firstBranch, out targetBranch);
				normal = targetBranch.GetNormalAtPosition (targetBranchPosition, rollAngle);
				return normal;
			}
			/// <summary>
			...
			/// </summary>
			/// <param name="positionAtBranchSkin">Position at BranchSkin.</param>
			/// <param name="firstBranch">First branch instance on this skin.</param>
			/// <param name="branchAtBranchSkin">Out parameter for the branch instance found having the position.</param>
			/// <returns>Position at a branch instance.</returns>
			public float TranslateToPositionAtBranch (
				float positionAtBranchSkin, 
				BroccoTree.Branch firstBranch, 
				out BroccoTree.Branch branchAtBranchSkin) {
				branchAtBranchSkin = null;
				// If the provided first branch instance is indexed as the first range on the branch ranges.
				if (id == firstBranch.id) {
					// If the position < 0
					if (positionAtBranchSkin < 0f) {
						branchAtBranchSkin = firstBranch;
						return (positionAtBranchSkin * length) / firstBranch.length;
					} else {
						float accumLength = 0f;
						float targetLength = length * positionAtBranchSkin;
						...
						do {
							if (accumLength + currentBranch.length > targetLength) {
								branchAtBranchSkin = currentBranch;
						...
						branchAtBranchSkin = prevBranch;
						return ((positionAtBranchSkin * length) - (length - branchAtBranchSkin.length)) / branchAtBranchSkin.length;
					}
				}
				return 0f;
			}
		...
		/// <summary>
		/// Used to get the branches curve points at the base of the tree. The lower the value more resolution, the higher lower resolution.
		/// </summary>
		public float branchAngleToleranceAtBase = 5f;
		/// <summary>
		/// Used to get the branches curve points at the top of the tree. The lower the value more resolution, the higher lower resolution.
		/// </summary>
		public float branchAngleToleranceAtTop = 15f;
		/// <summary>
		...
		/// <summary>
		/// Gets the first branch on a BranchSkin given the BranchSkin id.
		/// </summary>
		/// <param name="id">Id of the BranchSkin instance.</param>
		/// <returns>Branch instance if found, otherwise null.</returns>
		public BroccoTree.Branch GetBranchSkinFirstBranch (int id)
		{
			if (idToFirstBranch.ContainsKey (id)) {
				return idToFirstBranch [id];
			}
			return null;
		}
		/// <summary>
		...
		/// </summary>
		/// <returns></returns>
		public Dictionary<int, BranchSkin> GetIdToBranchSkin () {
			return idToBranchSkin;
		}
		/// <summary>
		...
		protected void PreprocessBranchSkin (BranchSkin branchSkin, BroccoTree.Branch firstBranch, BranchSkin parentBranchSkin = null, BroccoTree.Branch parentBranch = null) {
			...
			for (int i = 0; i < branchSkin.ranges.Count; i++) {
				...
				if (branchMeshBuilder != null) {
					branchMeshBuilder.SetAngleTolerance (branchAngleToleranceAtBase);
		...
		protected virtual float SkinBranchBase (BroccoTree.Branch branch, BranchSkin branchSkin) {
			...
			//CurvePoint curvePoint = branch.curve.GetPointAt (0.0001f);
			CurvePoint curvePoint = branch.curve.GetPointAt (0);
			if (useMeshCapAtBase) {
		...
		protected virtual float SkinBranchMiddleBody (BroccoTree.Branch branch, float consumedLength, BranchSkin branchSkin, bool isFollowup = false) {
			...
			*/
			...
			float hierarchyAtBase = branch.GetLevel() / (float)branch.parentTree.GetOffspringLevel();
			float hierarchyAtTop = (branch.GetLevel() + 1) / (float)branch.parentTree.GetOffspringLevel();

			List<CurvePoint> curvePoints = branch.curve.GetPoints (
				Mathf.Lerp (branchAngleToleranceAtBase, branchAngleToleranceAtTop, hierarchyAtBase),
				Mathf.Lerp (branchAngleToleranceAtBase, branchAngleToleranceAtTop, hierarchyAtTop),
				branchRelevantPositions);
			...
			for (int i = (isFollowup?0:1); i < curvePoints.Count; i++) {
				...
				//Debug.Log ($"curvePoints[{i}].relativePosition: {curvePoints[i].relativePosition}");
			...
			BranchSkin branchSkin)
		{
			...
			for (int i = 0; i <= numberOfSegments; i++) {
				branchSkin.AddShapeSegmentVertex (
					branch.GetSurfacePointAt (branchPosition, i * radialStep, false), 
					branch.GetSurfaceNormalAt (branchPosition, i * radialStep), 
					i * step);
			}
		}
		#endregion
		...
		IBranchMeshBuilder GetBuilderAtPosition (BranchSkin branchSkin, float position, out bool upperLimitReached, bool useInclusiveUpperLimit = true) {
			...
			_branchMeshBuilders [builderType].SetAngleTolerance (branchAngleToleranceAtBase);
			return _branchMeshBuilders [builderType];
		}
		#endregion
		...
		/// Creates a mesh using Jobs for all the BranchSkin instances found on this instance.
		/// </summary>
		...
		protected Mesh MeshBranchSkinsJob () {
			...
			for (int i = 0; i < totalBranchSkins; i++) {
				...
				totalShapeNormals += branchSkins [i].totalShapeNormals;
			}
			...
			BranchJob _branchJob = new BranchJob () {
				...
				aShapeNormals = new NativeArray<Vector4> (totalShapeNormals, Allocator.TempJob),
				...
				uv0s = new NativeArray<Vector4> (totalVertices, Allocator.TempJob),
				uv1s = new NativeArray<Vector4> (totalVertices, Allocator.TempJob),
			...
			for (int i = 0; i < branchSkins.Count; i++) {
				...
				// Iterate through segments to add them to the job.
				...
				// Add BranchSkin shape vertices and normals (if present).
				bool hasShapeNormals = _branchSkin.shapeNormals.Count > 0;
				for (int j = 0; j < _branchSkin.shapeVerticesPos.Count; j++) {
					...
					if (hasShapeNormals) {
						_branchJob.aShapeNormals [_shapeVerticesStartIndex + j] = _branchSkin.shapeNormals [j];
					}
				}
			...
			mesh.SetUVs (0, _branchJob.uv0s);
			mesh.SetUVs (1, _branchJob.uv1s);
			mesh.SetUVs (4, _branchJob.uv4s);
			mesh.SetUVs (5, _branchJob.uv5s);
			mesh.SetUVs (6, _branchJob.uv6s);
			mesh.SetUVs (7, _branchJob.uv7s);
			...
			_branchJob.aShapeNormals.Dispose ();

			_branchJob.vertices.Dispose ();
			_branchJob.normals.Dispose ();
			_branchJob.uv0s.Dispose ();
			_branchJob.uv1s.Dispose ();
			_branchJob.uv4s.Dispose ();
