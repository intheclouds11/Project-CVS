...
namespace Broccoli.Pipe {
	...
	public class Pipeline : ScriptableObject, ISerializationCallbackReceiver {
		...
		/// <summary>
		/// Source elements origin of valid pipelines.
		/// </summary>
		List<PipelineElement> validSrcElements = new List<PipelineElement> ();
		/// <summary>
		...
		#region Elements
		[System.NonSerialized]
		public PipelineElement selectedElement = null;
		/// <summary>
		/// Gets the elements.
		/// </summary>
		...
		public void OnAfterDeserialize () {
			...
			for (int i = 0; i < elements.Count; i++) {
				if (elements[i].connectionType == PipelineElement.ConnectionType.Source ||
					...
					if (elements[i].sinkElementIndex < 0) {
						elements[i].sinkElement = null;
						elements[i].sinkElementId = -1;
					} else {
						PipelineElement sinkElement = GetElementById (elements[i].sinkElementId);
						elements[i].sinkElement = sinkElement;
					}
				}
				if (elements[i].connectionType == PipelineElement.ConnectionType.Sink ||
					elements[i].connectionType == PipelineElement.ConnectionType.Transform) {
					if (elements[i].srcElementIndex <0) {
						elements[i].srcElement = null;
						elements[i].srcElementId = -1;
					} else {
						PipelineElement srcElement = GetElementById (elements[i].srcElementId);
						elements[i].srcElement = srcElement;
					}
				}
			}
			BuildKeyNameIndex ();
		...
		public bool Validate () {
			if (!_onValidateCalled) OnValidate ();
			if (elements.Count > 0) {
				srcElements.Clear ();
				validSrcElements.Clear ();
				for (int i = 0; i < elements.Count; i++) {
				...
				if (srcElements.Count == 0) {
					...
					if (validSrcElements.Count == 0) {
						state = State.NoSinkElement;
					} else {
						// Order by element probability.
						validSrcElements.Sort((x, y) => y.probability.CompareTo(x.probability));
						root = validSrcElements [0];
						state = State.Valid;
						for (int i = 0; i < validSrcElements.Count; i++) {
							SetElementsOnValidPipeline (validSrcElements [i]);	
						}
						return true;
					}
				}
		...
		/// <summary>
		/// Determines wheter this instance contains elements in it.
		/// </summary>
		/// <returns><c>True</c> if this pipelines has no elements.</returns>
		public bool IsEmpty () {
		...
		/// </summary>
		/// <param name="pipelineElement">Pipeline element.</param>
		private bool CheckElementConnection (PipelineElement pipelineElement, PipelineElement previousElement = null, PipelineElement srcElement = null) {
			...
			if (pipelineElement.sinkElement != null) {
				...
				return CheckElementConnection (pipelineElement.sinkElement, pipelineElement, srcElement);
			}
			return true;
		}
		/// <summary>
		...
		private void SetElementsOnValidPipeline (PipelineElement rootElement) {
			...
			{
				PipelineElement sinkElement = rootElement.sinkElement;
				sinkElement.srcElement = rootElement;
				sinkElement.srcElementClassType = rootElement.classType;
				sinkElement.srcElementId = rootElement.id;
				sinkElement.srcElementIndex = rootElement.index;
				SetElementsOnValidPipeline (sinkElement);
			}
		}
		/// <summary>
		...
		public Pipeline Clone (Pipeline clone = null) {
			...
			clone.seed = seed;
			// Reflect the elements to the serializedPipeline
			clone.OnBeforeSerialize ();
			return clone;
		}
		#endregion
		...
		/// </summary>
		/// <param name="srcElement">Source element to mark as preferred.</param>
		/// <returns><c>True</c> if the element was marked as preferred.</returns>
		public bool SetPreferredSrcElement (PipelineElement srcElement) {
			if (srcElement != null && srcElement.connectionType == PipelineElement.ConnectionType.Source) {
				for (int i = 0; i < srcElements.Count; i++) {
					srcElements[i].probability = 0f;
				}
				srcElement.probability = 1f;
				if (validSrcElements.Contains (srcElement)) {
					root = srcElement;
				}
				return true;
			}
