		...
		public override bool Validate () {
			...
			if (sproutMeshes.Count == 0) {
				...
				for (int i = 0; i < sproutMeshes.Count; i++) {
					...
					if (sproutMeshes[i].meshingMode == SproutMesh.MeshingMode.Shape && 
						sproutMeshes[i].shapeMode == SproutMesh.ShapeMode.Mesh && 
						sproutMeshes[i].meshGameObject == null)
					{
						allMeshAssigned = false;
						break;
					}
					if (sproutMeshes[i].meshingMode == SproutMesh.MeshingMode.BranchCollection && 
						sproutMeshes[i].branchCollection == null)
					{
