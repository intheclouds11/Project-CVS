...
namespace Broccoli.Pipe {
	...
	public class StructureGeneratorElement : PipelineElement, ISproutGroupConsumer, IStructureGenerator, ISerializationCallbackReceiver {
		...
		/// <summary>
		/// Gets all the SproutLevels (for Sprouts) assigned to a Group.
		/// </summary>
		/// <param name="groupId">Groupd Id</param>
		/// <returns>List of StructureLevel instances assigned to a Group.</returns>
		public List<StructureGenerator.StructureLevel> GetStructureLevesByGroupId (int groupId)
		{
			List<StructureGenerator.StructureLevel> results = new List<StructureGenerator.StructureLevel>();
			foreach (StructureGenerator.StructureLevel structureLevel in flatStructureLevels) {
				if (structureLevel.sproutGroupId == groupId) {
					results.Add (structureLevel);
				}
			}
			return results;
		}
