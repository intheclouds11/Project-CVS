...
namespace Broccoli.Pipe {
	...
	public abstract class PipelineElement : ScriptableObject {
		...
		/// such as trees or small plants, so that the GUI can display more tuned values to the 
		/// scale of the plant.
		/// </summary>
		public enum Preset {
			Tree,
		...
		/// <summary>
		/// The working preset for this element.
		/// </summary>
		public Preset preset = Preset.Tree;
		/// <summary>
		...
		public int id = -1;
		/// <summary>
		/// Probability for this element to be picker up as the main element on a group.
		/// </summary>
		...
		/// </summary>
		[System.NonSerialized]
		public string elementDescription = string.Empty;
		/// <summary>
		/// URL to the node documentation.
		/// </summary>
		[System.NonSerialized]
		public string elementHelpURL = string.Empty;
		/// <summary>
		/// Saves the show gizmos state for the pipeline element.
		/// </summary>
		public bool showGizmosEnabled = true;
		#endregion

		#region Events
		public delegate void OnPipelineElementDelegate ();
		public OnPipelineElementDelegate onChange;
		public OnPipelineElementDelegate onValidate;
		public void RaiseChangeEvent () {
			onChange?.Invoke ();
		}
		public void RaiseValidateEvent () {
			onValidate?.Invoke ();
		}
		#endregion
		...
		/// <summary>
		/// Resets the random seed only if fixed is the random mode.
		/// </summary>
		public void ResetFixedSeed () {
			if (isSeedFixed) {
				PrepareSeed ();
			}
		}
		...
		/// Sets a custom seed.
		/// </summary>
		/// <param name="customSeed">Random seed.</param>
		...
		public abstract PipelineElement Clone (bool isDuplicate = false);
		protected void SetCloneProperties (PipelineElement clone, bool isDuplicate) {
			if (isDuplicate)
				clone.id = -1;
			else
				clone.id = id;
