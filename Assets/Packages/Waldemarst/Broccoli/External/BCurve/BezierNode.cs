...
namespace Broccoli.Model
{
	...
	public class BezierNode : ISerializationCallbackReceiver {
		...
		/// <summary>
		/// Flag state to mark this node with. This is open to be used by
		/// an implementation as it better fits.
		/// </summary>
		[System.NonSerialized]
		public int flag = 0;
		/// <summary>
		...
		public Vector3 position {
			...
			set {
				_position = value;
				if (onChange != null && listenEvents) onChange (this, EventArgs.Empty);
			}
		}
		/// <summary>
		...
		public void ApplyTransform (float scale, Quaternion rotation) {
			// Apply scale and rotation to the position.
			_position = rotation * (_position * scale);
			lengthPosition *= scale;

			// Applu scale and rotation to handles.
			_handle1 = rotation * (_handle1 * scale);
			_handle2 = rotation * (_handle2 * scale);
		}
		#endregion
