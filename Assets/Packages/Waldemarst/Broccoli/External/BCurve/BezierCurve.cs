...
namespace Broccoli.Model
{
	...
	public class BezierCurve : ISerializationCallbackReceiver {
		...
		/// <summary>
		/// Fast Noise instance to apply noise to this curve.
		/// </summary>
		public FastNoiseLite noise = null;
		/// <summary>
        /// Noise resolution for length.
        /// </summary>
        public float noiseResolution = 4f;
        /// <summary>
        /// Noise strength, how much noise displaces each point.
        /// </summary>
		...
		/// <summary>
		/// Scale applied to the length noise.
		/// </summary>
		public float noiseLengthScale = 1f;
		/// <summary>
		...
		/// <summary>
		/// Girth value for the first node in this curve.
		/// </summary>
		public float girthAtFirstNode = 1f;
		/// <summary>
		/// Girth value for the last node in this curve.
		/// </summary>
		public float girthAtLastNode = 1f;
		/// <summary>
		/// Accumulated length value for the first node in this curve.
		/// </summary>
		public float accumLengthAtFirstNode = 0f;
		/// <summary>
		/// Accumulated length value for the last node in this curve.
		/// </summary>
		public float accumLengthAtLastNode = 0f;
		/// <summary>
		...
		public int resolution {
			...
			set {
				...
				//Debug.Log ("Resolution of " + value);
				int steps = 2;
				for (int i = 0; i < bezierCurves.Count; i++) {
					steps = Mathf.RoundToInt (bezierCurves[i].length * _cubicCurveSteps);
					//Debug.Log ("  steps " + steps);
					if (steps < 2) steps = 2;
					bezierCurves[i].stepCount = steps;
				}
			}
		}
		...
		#region Delegates
		/// <summary>
        /// Delegate to call when this curve changes.
		/// </summary>
		/// <param name="curve">Curve receiving the change.</param>
        public delegate void OnCurveDelegate (BezierCurve curve);
		/// <summary>
		/// Delegate to call when the length of this curve changes.
		/// </summary>
		/// <param name="curve">Curve receiving the curve change.</param>
		/// <param name="oldLength">Curve old length.</param>
		/// <param name="newLength">Curve new length.</param>
		public delegate void OnCurveLengthDelegate (BezierCurve curve, float oldLength, float newLength);
		/// <summary>
		/// Delegate to call on node events for a curve.
		/// </summary>
		/// <param name="curve">The curve owning the node.</param>
		/// <param name="node">Curve's node.</param>
		/// <param name="index">Index of the node on the curve.</param>
		/// <param name="processed"><c>True</c> if the curve was processed after the node event.</param>
		public delegate void OnNodeDelegate (BezierCurve curve, BezierNode node, int index, bool processed);
		/// <summary>
		/// Delegate to call on nodes events for a curve.
		/// </summary>
		/// <param name="curve">The curve owning the nodes.</param>
		/// <param name="nodes">List of Curve's nodes.</param>
		/// <param name="processed"><c>True</c> if the curve was processed after the nodes event.</param>
		public delegate void OnNodesDelegate (BezierCurve curve, List<BezierNode> nodes, bool processed);
		#endregion

		#region Events
		/// <summary>
        /// Event raised when a curve is about to change.
        /// </summary>
		[System.NonSerialized]
        public OnCurveDelegate onBeforeChange;
		/// <summary>
        /// Event raised when the curve changes.
        /// </summary>
		[System.NonSerialized]
        public OnCurveDelegate onChange;
		/// <summary>
		/// Event raised when the length of the curve changes.
		/// </summary>
		[System.NonSerialized]
		public OnCurveLengthDelegate onLengthChanged;
		/// <summary>
		/// Event raised before a nodes is added to the curve.
		/// </summary>
		[System.NonSerialized]
		public OnNodeDelegate onBeforeNodeAdd;
		/// <summary>
		/// Event raised when a nodes is added to the curve.
		/// </summary>
		[System.NonSerialized]
		public OnNodeDelegate onNodeAdd;
		#endregion
		...
		public void ProcessAfterCurveChanged () {
			...
			if (simplifyBias == SimplifyBias.Angle) {
				points = GetPoints (resolutionAngle, null);
			} else {
			...
			if (!Mathf.Approximately (oldLength,length)) {
				onLengthChanged?.Invoke (this, oldLength, length);
			}
			...
			if (normalMode == NormalMode.FixedNormal) {
				for (int i = 0; i < bezierCurves.Count; i++) {
					for (int j = 0; j < bezierCurves[i].samples.Count; j++) {
						...
						bezierCurves[i].samples[j].bitangent = Vector3.Cross (bezierCurves[i].samples[j].forward, fixedNormal);
					}
				}
				for (int i = 0; i < points.Count; i++) {
					points[i].normal = fixedNormal;
					points[i].bitangent = Vector3.Cross (points[i].forward, fixedNormal);
				}
			} else {
				...
				for (int i = 0; i < bezierCurves.Count; i++) {
					for (int j = 0; j < bezierCurves[i].samples.Count; j++) {
						if (normalMode == NormalMode.ReferencePoint) {
							_normal = Vector3.ProjectOnPlane (bezierCurves[i].samples[j].position, _forward).normalized;
						}
						...
						bezierCurves[i].samples[j].normal = _normal;
						bezierCurves[i].samples[j].bitangent = Vector3.Cross (bezierCurves[i].samples[j].forward, bezierCurves[i].samples[j].normal);
					}
				}
				for (int i = 0; i < points.Count; i++) {
					if (normalMode == NormalMode.ReferencePoint) {
						_normal = Vector3.ProjectOnPlane (points[i].position, _forward).normalized;
					}
					rotation = Quaternion.FromToRotation (_forward, points[i].forward);
					points[i].normal = rotation * _normal;
					points[i].bitangent = Vector3.Cross (points[i].forward, points[i].normal);
				}
			}
		}
		...
		public void SetNoise (
			...
			float girthAtFirstNode,
			float girthAtLastNode,
			float accumLengthAtFirstNode,
			float accumLengthAtLastNode,
			bool spareNoiseOffsetAtFirstPoint,
			float noiseResolution = 4f,
        	float noiseStrength = 0.2f,
			float noiseLengthOffset = 0f,
			float noiseLengthScale = 1f)
		{
			...
			this.girthAtFirstNode = girthAtFirstNode;
			this.girthAtLastNode = girthAtLastNode;
			this.accumLengthAtFirstNode = accumLengthAtFirstNode;
			this.accumLengthAtLastNode = accumLengthAtLastNode;
			this.noiseLengthOffset = noiseLengthOffset;
			this.noiseLengthScale = noiseLengthScale;
			this.spareNoiseOffsetAtFirstPoint = spareNoiseOffsetAtFirstPoint;
			float accumLength = 0f;
			for (int i = 0; i < bezierCurves.Count; i++) {
				bezierCurves [i].noise = noise;
				if (i == 0) {
				...
				bezierCurves [i].noiseResolution = noiseResolution;
				bezierCurves [i].noiseStrength = noiseStrength;
				bezierCurves [i].noiseLengthOffset = noiseLengthOffset + accumLength;
				bezierCurves [i].noiseLengthScale = noiseLengthScale;
				...
				bezierCurves [i].girthAtFirstNode = Mathf.Lerp (girthAtFirstNode, girthAtLastNode, accumLength / (float)length);
				bezierCurves [i].accumLengthAtFirstNode = Mathf.Lerp (accumLengthAtFirstNode, accumLengthAtLastNode, accumLength / (float)length);
				...
				bezierCurves [i].girthAtLastNode = Mathf.Lerp (girthAtFirstNode, girthAtLastNode, accumLength / (float)length);
				bezierCurves [i].accumLengthAtLastNode = Mathf.Lerp (accumLengthAtFirstNode, accumLengthAtLastNode, accumLength / (float)length);
			}
		}
		/// <summary>
		/// Reapplies noise values to the children curves.
		/// </summary>
		public void SetNoise () {
			SetNoise (
				noiseFactorAtFirstNode, 
				noiseFactorAtLastNode, 
				noiseScaleAtFirstNode, 
				noiseScaleAtLastNode,
				girthAtFirstNode,
				girthAtLastNode,
				accumLengthAtFirstNode,
				accumLengthAtLastNode,
				spareNoiseOffsetAtFirstPoint,
				noiseResolution,
				noiseStrength,
				noiseLengthOffset, 
				noiseLengthScale);
		...
		/// </summary>
		/// <param name="angleTolerance">Angle tolerance to filter points of the curve.</param>
		/// <param name="relevantPositions">List of positions to add for relevance instead of their angle filtering.</param>
		/// <returns>List of curve points.</returns>
		public List<CurvePoint> GetPoints (float angleTolerance = 2f, List<float> relevantPositions = null) {
			return GetPoints (angleTolerance, angleTolerance, relevantPositions);
		}
		/// <summary>
		/// Get a list of curve points using angle tolerance to optimize the number of points.
		/// </summary>
		/// <param name="angleToleranceAtStart">Angle tolerance to filter points near the start of the curve.</param>
		/// <param name="angleToleranceAtEnd">Angle tolerance to filter points near the end of the curve.</param>
		/// <param name="relevalPositions">List of positions to .</param>
		/// <returns>List of curve points.</returns>
		public List<CurvePoint> GetPoints (float angleToleranceAtStart = 2f, float angleToleranceAtEnd = 2f, List<float> relevantPositions = null) {
				...
				for (int j = firstIndex; j < bezierCurves[i].samples.Count; j++) {
					...
					if (prevPoint != null) {
						...
						if (hasPrevVector) {
							...
							angleTolerance = Mathf.Lerp (angleToleranceAtStart, angleToleranceAtEnd,
								(accumLength + prevPoint.lengthPosition) / this.length);
		...
		/// <summary>
		/// Adds a node at the end of the list of nodes for this curve.
		/// </summary>
		/// <param name="node">Node to add.</param>
		/// <param name="autoProcess"><c>True</c> to autoprocess the curve after the node has been added.</param>
		public void AddNode (BezierNode node, bool autoProcess = true) {
			onBeforeNodeAdd?.Invoke (this, node, -1, autoProcess);
			nodes.Add (node);
			node.curve = this;
			if (autoProcess) {
				Process ();
				onNodeAdd?.Invoke (this, node, nodes.Count - 1, autoProcess);
			}
		}
		/// <summary>
		/// Inserts a node at the given index on the list of nodes for this curve.
		/// </summary>
		/// <param name="index">Index to insert the node at.</param>
		...
		public void InsertNode (int index, BezierNode node, bool autoProcess = true) {
			...
			if (autoProcess) {
				Process ();
				onNodeAdd?.Invoke (this, node, index, autoProcess);
			}
		}
		...
		*/
		...
		public void ApplyRotation (Quaternion rotation) {
			ApplyTransform (1f, rotation);
		}
		public void ApplyScale (float scale) {
			ApplyTransform (scale, Quaternion.identity);
		}
		public void ApplyTransform (float scale, Quaternion rotation) {
			referenceForward = rotation * referenceForward;
			referenceNormal = rotation * referenceNormal;

			// Apply scale and rotation to cubic bezier curve.
			for (int i = 0; i < bezierCurves.Count; i++) {
				bezierCurves [i].ApplyTransform (scale, rotation, i > 0);
			}

			// Apply scale and rotation to points.
			for (int i = 0; i < points.Count; i++) {
				points [i].position = rotation * (points [i].position * scale);
				points [i].forward = rotation * points [i].forward;
				points [i].normal = rotation * points [i].normal;
				points [i].bitangent = rotation * points [i].bitangent;
				points [i].tangent = rotation * points [i].tangent;
				points [i].lengthPosition *= scale;
			}

			// Apply scale and rotation to vector points.
			for (int i = 0; i < vectorPoints.Count; i++) {
				vectorPoints [i] = rotation * (vectorPoints [i] * scale);
			}

			// Apply scale to length.
			length *= scale;

			// Apply scale to noise.
			noiseFactorAtFirstNode *= scale;
			noiseFactorAtLastNode *= scale;
			noiseLengthScale /= scale;
			girthAtFirstNode *= scale;
			girthAtLastNode *= scale;
			SetNoise ();
		}
		public static List<CurvePoint> MergeCurvePointsByDistance (List<CurvePoint> points, float mergingPositionDistance) {
			...
			for (int i = 0; i < points.Count; i++) {
				...
				if (shouldMerge) {
					...
					points [referenceIndex].bitangent = (points [referenceIndex].bitangent + points [i].bitangent) / 2f;
					shouldMerge = false;
				}
		...
		public BezierCurve Clone (bool generateNewGUID = false) {
			...
			clone.noiseLengthScale = noiseLengthScale;
		...
		public BezierCurve Copy () {
			...
			copy.noiseLengthScale = noiseLengthScale;
