...
namespace Broccoli.Model {
    ...
    public class CubicBezierCurve {
        ...
        */
        ...
        public bool hasNoise {
            get {
                if (noise != null)
                    ...
                    return noiseFactorAtFirstNode != 0 || noiseFactorAtLastNode != 0;
            }
        }
		/// <summary>
        ...
        /// <summary>
        /// Scale applied to the length calculation of noise.
        /// </summary>
        ...
        /// <summary>
        /// Accumulated length at the first node.
        /// </summary>
        public float accumLengthAtFirstNode = 0f;
        /// <summary>
        /// Accumulated length at last node.
        /// </summary>
            ...
            if (hasNoise) {
                ...
                if (samples.Count >= 3) {
                    for (int i = 1; i < samples.Count - 1; i++) {
                        _f = samples [i].forward;
                        samples [i].forward = Vector3.Lerp (_f,
                            (((samples [i].position - samples [i - 1].position) + (samples [i +1].position - samples [i].position)) / 2f).normalized,
                            0.5f);
                    }
                }
            }
        ...
        /// <summary>
        /// Applies scaling and rotation to the curve.
        /// </summary>
        /// <param name="scale">Scaling value.</param>
        /// <param name="rotation">Rotation value.</param>
        /// <param name="skipFirst">If <c>true</c> then the first node does not receive the transform.</param>
        public void ApplyTransform (float scale, Quaternion rotation, bool skipFirst = false) {
            // Apply scale and rotation to nodes.
            if (!skipFirst) n1.ApplyTransform (scale, rotation);
            n2.ApplyTransform (scale, rotation);

            // Apply scale and rotation to samples.
            for (int i = 0; i < samples.Count; i++) {
                samples [i].position = rotation * (samples [i].position * scale);
                samples [i].forward = rotation * samples [i].forward;
				samples [i].normal = rotation * samples [i].normal;
				samples [i].bitangent = rotation * samples [i].bitangent;
				samples [i].tangent = rotation * samples [i].tangent;
                samples [i].lengthPosition *= scale;
            }
        ...
        private void ApplyNoise (CurvePoint curvePoint) {
            ...
            float noiseScale = Mathf.Lerp (noiseScaleAtFirstNode, noiseScaleAtLastNode, curvePoint.relativePosition);
            if (noise == null) {
                float pointNoiseX = ((curvePoint.lengthPosition + noiseLengthOffset) * 
                    noiseLengthScale * 
                    noiseScale);
                float pointNoiseY = ((curvePoint.lengthPosition + noiseLengthOffset + 0.5f) * 
                    noiseLengthScale * 
                    noiseScale);
                offset = new Vector3 (
                    (Mathf.PerlinNoise (pointNoiseX * noiseResolution, 0) - 0.5f) * noiseStrength,
                    0f,
                    (Mathf.PerlinNoise (0, pointNoiseY * noiseResolution) - 0.5f) * noiseStrength
                );
                offset *= Mathf.Lerp (noiseFactorAtFirstNode, noiseFactorAtLastNode, curvePoint.relativePosition);
            } else {
                float noiseStrength = Mathf.Lerp (noiseFactorAtFirstNode, noiseFactorAtLastNode, curvePoint.relativePosition);
                float girth = Mathf.Lerp (girthAtFirstNode, girthAtLastNode, curvePoint.relativePosition);
                float length = Mathf.Lerp (accumLengthAtFirstNode, accumLengthAtLastNode, curvePoint.relativePosition);
                float noiseAngle = noise.GetNoise (0, 0, (length + noiseLengthOffset) * noiseScale) * Mathf.PI;
                float noiseGrad = noise.GetNoise (0, 0, (length + noiseLengthOffset + 0.5f) * noiseScale) * 0.5f + 0.5f;
                offset = new Vector3 (Mathf.Cos(noiseAngle) * noiseGrad, 0f, Mathf.Sin(noiseAngle) * noiseGrad);
                offset *= girth * noiseStrength;
            }
